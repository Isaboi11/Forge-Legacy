...
- [ ] `expo prebuild` succeeds; the generated project contains the target. ⚠ **Cannot be run on this machine** — Expo skips iOS generation on Windows outright (*"Run npx expo prebuild again from macOS or Linux"*), and there is no WSL or Docker here. The plugin's source was read instead: the `watch` type sets `SDKROOT=watchos`, `TARGETED_DEVICE_FAMILY=4`, `WATCHOS_DEPLOYMENT_TARGET`, and `INFOPLIST_KEY_WKCompanionAppBundleIdentifier` from the main target, and embeds via "Embed Watch Content". First real proof is the EAS build.
- [ ] `eas build -p ios --profile production` signs **both** targets; `fingerprint:compare` shows the fingerprint changed (it must — this is native). ⚠ **Needs the PO's go**: the first build creates two new App IDs on the Apple developer account and will prompt for Apple credentials — not something to run unattended. It also has to carry the splash fix that already owes build 8, so the build number and the other in-flight work are coordinated first.
