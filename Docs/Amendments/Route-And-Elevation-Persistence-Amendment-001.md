...
| Version | Date | Change |
|---|---|---|
| v1.1 | 2026-08-26 | **D-RTE-1, D-RTE-2 and D-RTE-5 superseded by `Route-Sharing-Amendment-001.md`.** The PO vetoed the endpoint trim ("Take this out completely") and approved route sharing on posts and the shared Activity Detail, opt-in per post. D-RTE-3, D-RTE-4 and D-RTE-6 stand. Routes stored while v1.0 governed are permanently trimmed; the full track exists only for runs saved after the untrimmed client. |
