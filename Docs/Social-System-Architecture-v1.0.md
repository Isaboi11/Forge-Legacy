...
- **No popularity metric** — reaction/comment counts are never aggregated into a ranking and never appear on a profile as a status number. Engagement exists to **encourage conversation**, not to manufacture attention. ⚠ **AMENDED by `Social-Architecture-Amendment-004-Acknowledgement-Count.md` (LOCKED, August 2026):** a post **may** display its own acknowledgement and comment counts, beside the controls, hidden at zero. The distinction is between a number that describes a *post* and a number that scores a *person* — the second is still forbidden, absolutely, along with every ranking, sorting and profile use listed above. Amendment 004 also records that the four acknowledgement kinds moved to a press-and-hold on the single-tap Acknowledge control (SOC-A4-D3).
