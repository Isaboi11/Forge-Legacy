...
| **Restaurants / branded misses** | **FatSecret Basic (free tier)**, via an Edge Function | $0, restaurants. ⚠ **5,000 API calls/day, not unlimited** — corrected 2026-09-23 at key signup. Irrelevant under the preview allowlist; a real ceiling before Nutrition opens up, which is what the 24h `food_catalog` cache is for. **Premier Free** (62 countries, higher ceiling) is a separate application we have NOT filed — an earlier note here assumed we were on it. | ✅ **FatSecret confirmed in writing (2026-09-22, `Docs/Legal/FatSecret-Storage-Permission-2026-09-22.md`) that a diary entry may keep its calories and macros permanently.** Micronutrients were not named — re-read those by `food_id`. Everything *else* from FatSecret (search results, food detail) still follows the 24h rule: cache IDs, not data. Attribution "Powered by fatsecret". Free only under US$1M revenue — a cliff at roughly 40k MAU. **FatSecret is in Phase 1** (restaurants + branded misses). ⚠ **Written but dormant:** tokens are issued only to allowlisted IPs and Edge Functions egress from rotating addresses. FatSecret confirmed in writing (same file, second reply) that a static-IP relay is **not** required — `0.0.0.0/0` is permitted. Two PO console steps, no code: allowlist `0.0.0.0/0`, then set `FATSECRET_CLIENT_ID` / `FATSECRET_CLIENT_SECRET`. |
