...
>
> `AGENTS.md` requires every session to read the Master Status dashboard **before any planning,
> implementation or audit work**. By 2026-08-18 that dashboard had reached **5,374 lines / 868 KB**, and
> **81% of it was Recently Completed** — 48 entries, all written in the single week of 11–18 August. Every
> session was paying for a week of changelog before it could do anything, and the rate was compounding.
>
> The dashboard keeps the 15 most recent entries, which is what "recent" is for. Everything older moved
> here **verbatim** — not summarised, not trimmed, not rewritten.
>
> ## Why nothing was deleted
>
> Rule 2 of the dashboard's own workflow is *never delete completed milestones*, and it is right: several
> of these entries are the ONLY record of why something was built the way it was. They are load-bearing
> for anyone who later asks "why does it do that?" — which is exactly the question a stale ledger causes.
>
...
**The testers have the build, and have had it for a while (PO).** Both launch documents carried that as ⛔ and as *"the longest-outstanding item"* — `Docs/GO-LIVE.md` step 1 and checklist §8.8. It was wrong, so **Stage 1 is closed** and D-U-N-S (D&B case `10803372`, documents answered 08-15) is the only thing gating the public release. Everything else in Stage 2 is work that fits inside the wait.

**Six items were stale, and every one in the same direction.** `8.3` and `8.5` asked for migrations applied by 08-13; the database runs through `0162`. `8.7` asked for `buildNumber` 5; `app.json` is at 6. `9.1b` and `10.1` were closed by the site going live on 08-16. **`10.2` — the privacy-copy fix, listed as blocking the App Privacy labels — was closed by `cc2b5de` on 08-15**, and closed in the right order: the collection list was corrected *before* any label was signed, which is the only thing that item ever protected. Verified this pass that `site/privacy.html` and the in-app summary in `content.ts` agree in substance — seven collection categories, all three former omissions (precise location, photos and video, usage analytics) now stated.

**⚠ This board's characteristic failure is the mirror of the migration ledger's, and it is not harmless.** The ledger once listed eleven applied migrations as pending; these documents list finished work as pending. Neither has ever claimed unfinished work was done — the error has a direction, and the direction is what makes it expensive: §8.1 records the day nearly spent re-applying migrations already in, and **re-pasting an applied migration is not free** (`0141` died on `42P13`). *These files are written by hand and the repo is not.*

**⚠ THE FINDING: "free while testing" lives in three places and Phase F named one.** Every instance is true today and becomes a **false billing claim** the instant `default_tier` flips — and **nothing on any surface will look wrong**. Beyond `site/index.html` and its JSON-LD `offers: price "0"`, it is in **`src/domain/settings/content.ts:30,32`** — *"There is no subscription, no billing, and nothing to cancel"*, rendered **in-app**, on the membership sheet, to the exact cohort being charged — and it is **asserted by `src/domain/settings/__tests__/content.test.mjs:168`**, so **Phase F fails the test gate until the assertion is updated**. That is the gate working: the same file already bans `renews yearly`, `next charge`, `cancel at any time` and `Founder`, because shipped comp copy once told every tester all four falsely. Written up as new checklist item **6.2b**; the assertion gets updated to the new truth, never deleted.

**✅ Opened and closed the same day: the testers are on build 6** (PO, 2026-08-17) — the exact runtime (`411fd2b6…`, commit `aaee846c`) every OTA since 08-15 has been published against, so they are **current** on the units fix, the white screen, the squads outage, the chapter dead end and both cardio fixes. Worth asking rather than assuming: an OTA only reaches a device whose runtime matches, so on build 4 or 5 they would have received nothing since and no OTA could have repaired it. **⇒ The tester cohort is OTA-reachable, which is an asset with an expiry date — the first native module added ends it.** That is precisely the cost sitting on §4.2's RevenueCat install, and it is now a decision rather than a routine step.

**⚠ A correction to this pass's own advice.** It warned that the uncommitted `scripts/animation-processing/` files *"ship to phones on the next publish."* **They do not.** Metro bundles from the entry point, nothing in `src/` imports those scripts (only two comments reference the directory by name), so they are not in the JS any device runs. The standing rule — *publishing bundles the working tree, not HEAD* — is real, but its blast radius is **uncommitted code under `src/`**, and `src/` is clean. Committing them is hygiene, not a publish blocker. Scanned for a service key before saying so: none present.

**Files:** `Docs/GO-LIVE.md` → v1.2 · `Docs/Launch-Checklist-Free-And-Premium.md` → v1.2 (six ✅, one new item) · this board.

### 0. The same cardio report, through the door yesterday's fixes could not reach — a typed-in walk saved as "Freestyle Workout", 28 minutes (2026-08-17, W-9/W-21 — CODE only, no migration, OTA-safe)

**PO: *"Kim did a treadmill workout. She manually inputed her results for a 20 min walk. It logged it as a freestyle workout and it saved as 28 minutes."*** Both halves reproduce exactly, and **neither is a regression from entry 1 below** — that pass simply never reached this door. The two fixes there both assume the session was **named for its bout at the door** (Home's cardio chooser, Workouts' *Track a Run*). Kim came in through the other one: **"Start a freestyle workout"**, then added the walk from the picker.

**(1) The name.** A freestyle session is called the literal `Freestyle Workout` before it holds anything, and `setCardioModality`'s new rename is guarded on `cur.workoutName === was` — the **block's own previous derived name**. `Freestyle Workout` is not that, so the rename never fires and a treadmill walk is filed under a name describing nothing. **Fixed at SAVE instead of at the toggle**, which is what makes it door-independent: `sessionWorkoutName` (`save-core.ts`) runs once over the session's final shape, so it cannot be defeated by which control the athlete happened to touch — or by their never having touched one. Guarded three ways: it only ever replaces the literal placeholder (no control writes it and W-17's rename cannot produce it, so any other name was *chosen*), only for a session that is **one recorded cardio block** (a walk plus squats stays freestyle — naming it "Treadmill Walk" would name the smaller half), and only over `recordedExercises`, so an un-ended bout cannot name a session it is not part of. `continueWorkout` is deliberately untouched: it appends to a row that already has a name.

**(2) The clock.** `max(wall, logged)` only ever moves the number **up**. It fixed the wall clock being far too SHORT (90 minutes typed in over forty seconds) and cannot fix the mirror image: open the app, walk with it in a pocket or not at all, come back 28 minutes later and type 20 — `max(28, 20)` is 28. ⚠ **The bare wall clock was the same 28, so this is old, not new.** The wall clock earns its authority from the app having **watched** the session, and `source` already records exactly that on every bout (`'tracked'` vs `'manual'`, the two doors the card offers on purpose). **`typedInSession` now returns the logged time alone when every recorded bout says `'manual'`.** Narrow by construction: ALL of them (one tracked bout puts the wall clock back in charge of the whole session), **cardio only** via `recordedExercises` (strength writes no bout, so crediting only what was typed would discard the time spent lifting), and `'manual'` **explicitly** — a bout with no `source` keeps the max, because absence is not a claim. ⚠ **And this is the one place a SUM is right**: the max exists because bouts sit *inside* a session that held them; here nothing was measured, so two walks typed in as 20 minutes each are forty minutes of walking.

**⚠ `activity_type` was never wrong** — `sessionActivityType` correctly derived `walking` the whole time. Only the display name lied, which is why it read as "logged as a freestyle workout" rather than as a miscategorised session.

**⚠ Not retroactive**, same as entry 1: Kim's existing row keeps 28 minutes and the freestyle name. The name is editable by her (W-17's rename, and the activity detail screen); the duration is not.

`tsc` 0 · **2,471 tests green** (10 new in `cardio-manual-log.test.mjs`) · lint at baseline (1 error + 13 warnings, both pre-existing) · **both fixes mutation-tested** — disabling each individually fails exactly its own tests and nothing else. One of the new tests was strengthened after the mutation pass showed it passed either way (its wall clock had been shorter than the sum it was asserting).

**Existing rows: `supabase/apply/fix-manual-cardio-rows.sql`, and it is a PREVIEW then an UPDATE on purpose.** `source` lives only in the client session shape and has never been written to any column, so SQL genuinely cannot tell a typed bout from a tracked one — and the difference is exactly what decides whether a row is wrong (a tracked bout's wall clock legitimately runs a little longer than the bout). STEP 1 lists candidates with the gap in minutes so the judgement is a human one; STEP 2 corrects only the ids named. ⚠ Do not collapse it into a blanket update — it would shave real seconds off every tracked run in the table. **RUN 2026-08-17 against `kimjovi`** — STEP 1 returned exactly one candidate (stored **1664s** against a **1200s** bout, gap **7.7 min**, named `Freestyle Workout`), which is the reported row and the only one; STEP 2 corrected it to 20 minutes and "Treadmill Walk". No other athlete has been swept, and the file keeps its placeholder ids rather than Kim's — a workout id is personal data and does not belong in the repo.

**DEPLOYED AND VERIFIED, both surfaces (2026-08-17).** Web: `forgelegacy.expo.app` **200**, serving `entry-9ff10d1a58786966e374fece5498a5d1.js` — **md5 byte-identical to local `dist`** (Expo names the file by its md5, so filename == hash is the check). ⚠ The deploy log again went `Preparing project → Creating deployment` **with no upload step** — this project's recorded tell for an empty deploy — so it was verified rather than trusted, and was fine. Both fixes are present in the downloaded live file: `sessionWorkoutName` survives minification as an export (×2), while `typedInSession` is module-local and does not — grep instead for the minified guard **`&&'manual'===t.cardio?.source)}const o='Freestyle Workout'`**, which is both fixes in one span. Phone: `eas update --branch production --environment production` published on iOS runtime **`411fd2b68cbe…`**, which *is* build 6's (`078d2838`, commit `aaee846c`) — the runtime version in the output is the only evidence of deliverability. Android published separately on `a8afa07c…`. ⚠ **`fingerprint:compare` was run BEFORE publishing and reported `✅ matches`** — the OTA trap from entry 1 stayed closed despite an untracked `scripts/animation-processing/` tree in the working directory. The `*` on the commit in the publish output is that same uncommitted tree; none of it is under `src/`, so none of it is in the bundle.

### 0. Two PO reports on the cardio card — a hand-entered bout lost its clock, and a treadmill walk was filed as an outdoor one (2026-08-17, W-9/W-21 — CODE only, no migration, OTA-safe)

**(1) *"My last bike I logged for 90 min and it said less than one minute."*** `workouts.duration_sec` was `Date.now() - session.startedAt` and nothing else. That is honest for a workout performed with the app open, and it is a lie about the two paths the cardio card offers ON PURPOSE — *"Skip timer · enter it myself"* and *"Already did it · log manually"* — both of which exist so a bout the app never watched can still be recorded. Take either, type 90 minutes, tap Finish, and the stored duration is the forty seconds of typing, which `fmtDuration` floors to the literal words the PO read back: **"< 1 min"**. It was never only a label: W-17 divides `workouts.distance` by this column to report the session's pace, so a 15-mile ride filed as 45 seconds also announced a personal best at four figures of mph. **`sessionDurationSec` (`save-core.ts`) now takes the LONGER of the wall clock and the bouts the athlete filed** — a max, never a sum, because two bouts with rest between them are shorter than the session that held them. This mirrors `continue_workout`'s own `greatest(duration_sec, p_duration_sec)` rule on the same column, so the two writers cannot disagree about which way the number moves. Cardio only: a timed strength set writes `duration_sec` too (a 60s plank), and summing holds against the session they were part of would be the same class of error pointed the other way. Used by BOTH `saveWorkout` and `continueWorkout`.

**(2) *"It's logging it as an outdoor walk when it's a treadmill."*** A cardio-only session is **named at the door** — Home's "Something else today?" calls `startWorkout(deriveName(activity, CARDIO_DEFAULTS[activity].modality))` and Workouts' *Track a Run* hard-codes `'Outdoor Run'` — while the Outdoor/Treadmill toggle lives on the card one screen later, deliberately, because nobody decides indoors or outdoors from a menu. `setCardioModality` renamed the **block** and left the **workout** alone, so every treadmill walk started that way was filed under "Outdoor Walk": the session's name asserted the one thing about it that was not true. It now re-derives `workoutName` too — **guarded twice**: only for a one-block session, and only while the name still equals the block's own previous derived name. `⋯ Options → name this workout` writes the same field, and a name the athlete typed is theirs.

**⚠ Neither fix is retroactive.** Sessions already saved carry the wrong duration and the wrong name on the row. The name is editable by the athlete (W-19's rename); the duration is not, and correcting existing rows would need a migration nobody has asked for.

`tsc` 0 · **2,461 tests green** (11 new in `cardio-manual-log.test.mjs`) · lint at baseline (1 error + 13 warnings, both pre-existing) · **both fixes mutation-tested** — reverting each one individually fails exactly the tests written for it.

**⚠ AND THE OTA WAS BLOCKED, BY A GITIGNORE LINE — the same trap, for the second time, and now resolved (`e3eba5e`).** `fingerprint:compare` against build 6 (`411fd2b6…`, commit `aaee846c`, 08-15) named **one** differing path: `modified file: .gitignore`. The landing-page pass (`2439b11`) had added `site/_exported-bundle.html` to it, and `@expo/fingerprint` hashes `.gitignore` itself — so every OTA since 08-16 would have landed on a runtime no build was running and reported `✔ Published!` to nobody. **The rule moved to `.git/info/exclude`** rather than being deleted: not a repo file, not hashed, and `git check-ignore -v` confirms the 4 MB export is still ignored. Trimmed **bytewise to preserve CRLF** — rewriting the file from `git show`'s LF blob would have produced a different hash with a diff that looks empty. `git diff aaee846c -- .gitignore` is now empty and `fingerprint:compare` reports `✅ matches`. ⚠ `.git/info/exclude` is local to this clone; a fresh clone needs the line re-added *there*, never back in `.gitignore`.

**DEPLOYED AND VERIFIED, both surfaces (2026-08-17).** Web: `forgelegacy.expo.app` **200**, serving `entry-bc06e478a897539d37f41e92710cb7ae.js` — **md5 byte-identical to local `dist`**, and the downloaded live file carries both fixes (`sessionDurationSec` ×2, and the minified guard `workoutName:1===t.exercises.length&&t.workoutName===o?r:…`; note the minifier Yoda-flips `1===`, so grep for that form). `/log-activity` and `/workout` both 200; the Supabase URL is present in the live bundle. ⚠ The deploy log went `Preparing project → Creating deployment` **with no upload step**, which is this project's recorded tell for an empty deploy — it was verified rather than trusted, and was fine. Phone: `eas update --branch production` published on iOS runtime **`411fd2b68cbe…`**, which *is* build 6's — the runtime version in the output is the only evidence that matters. Android published separately on `a8afa07c…`. **This carries P-8 too** — by PO decision, and it is how the paywall screen gets its first browser render.

### 0. P-8 Subscription is built — the paywall screen exists, the store does not yet (2026-08-16, Launch Checklist 4.1 — CODE only, no migration, OTA-safe)

**`src/app/subscription.tsx` is the paywall.** Both locked entry contexts work: a push from Account Settings draws a back chevron; M-7's Upgrade dismisses and pushes `?from=gate`, which draws a [×] returning to whichever surface triggered the gate. `SUBSCRIPTION_ROUTE_BUILT` is flipped in `useCeremony.tsx` — the honest "Subscriptions open with the next release" toast is retired. Four states: loading, Free, Premium, and **`unknown`**, which the spec does not model and which must not default either way — drawing Free would upsell a Premium athlete whose network dropped (M-7 §10 forbids exactly that), drawing Premium would tell a Free athlete they had paid.

**Three new pieces, and the shape matters more than the screen.** `src/domain/billing/plans-core.ts` is pure and holds every rule the spec numbers — annual pre-selected (P8W-D2), lifetime never pre-selected (P8W-D8), the Founder row hidden unless the seat count is *both read and positive* (P8W-D5), the saving computed from two platform prices or rendered as nothing at all (P8W-D3). **23 tests**, including one that reads `0145` and fails if the client's Founder denominator ever drifts from `entitlement_config.founder_seats_total` — *"first 100"* is a promise, and a false scarcity claim is a deceptive practice (MA3-D24). `src/lib/billing.ts` is the store port. `fetchCapConfig()` reads **both** cap columns, because `my_entitlement()` correctly returns only the caller's — and P-8 is the one surface that must show the other tier's numbers without inventing them.

**⚠ RevenueCat is deliberately NOT installed, and the screen ships anyway.** `react-native-purchases` is a native module: the moment it lands, every OTA to the build in testers' hands stops being deliverable until a new binary ships, and **Stage 1 needs no paywall code at all**. So P-8 is written against `BillingAdapter`; `UNAVAILABLE_BILLING` is in force and the screen says so honestly rather than faking a price. 4.2 is now an adapter, not a rewrite. **Manage Subscription already works** — it is a deep link, not a transaction.

**⚠ The `.dc` and the locked spec disagreed, and the resolution was a PO decision: the design governs the visual language, the spec governs every number, plan and claim.** `Forge Subscription.dc.html` predates the 2026-08-12 pricing lock — three plans at **typed** prices, no Founder or Lifetime row, no Coach AI disclosure, fineprint carrying the exact renewal claims `content.test.mjs` guards against, and benefits promising analytics, Communities and *"unlimited Squads"* when Premium's ceiling is genuinely 5 (M7-D15 forbids that claim outright). The design's hero, plan cards, benefit rows, at-a-glance grid and commit bar are kept; its commercial content is not. The `.dc`'s social-proof line was **dropped** — an unverifiable claim on a purchase screen for a product with 20 testers.

**⚠ Two things are owed.** (1) **`Docs/P-8-Subscription-Wireframe-Spec.md` contradicts itself**: §11.2's prose puts the picker "between the reassurance line and the usage review", its own §3.2 diagram puts it above the buy button. Built to the diagram — plan → disclosure → button adjacency *is* P8W-D4's purpose — but the spec needs an amendment rather than a silent resolution. (2) **The screen has not been rendered in a browser.** `tsc` 0 · **2,450 tests green** · lint at baseline (1 error + 13 warnings, both pre-existing) · the web bundle builds and emits the route — but static export is shell-only, so nothing has *seen* it. That check is owed before anyone is shown it.

### 0. `forgelegacy.app` resolves — the Apple enrollment gate is cleared, and D-U-N-S is now the only blocker (2026-08-16, OPS — no code, no migration, not an OTA)

**The site built the day before is live.** Root **200** (155,105 bytes, byte-for-byte `index.html`), `/privacy` **200**, `/terms` **200**, `www.forgelegacy.app` **200**, `http://` **301 → https** on both hosts. **The MX records are intact** — `isaiah@` and `support@` were never at risk, because an address record and a mail record are different record types on the same name. Apple's organization-enrollment requirement (a public, functional website on a domain associated with the organization; parking pages and thin sites explicitly rejected) **is satisfied**. Enrollment now waits on D&B case **10803372** and nothing else.

**⚠ It is a Cloudflare WORKER, not Pages, and every document in this repo said Pages.** Cloudflare's *Create application* flow now routes static uploads to Workers-with-static-assets and demotes Pages to a *"Looking to deploy Pages?"* footnote. Both would have served the files; the Worker is the non-deprecated path. `site/README.md` and `Docs/GO-LIVE.md` are corrected. **The project is named `forgelegacy`** — worth recording because **a Worker cannot be renamed**: the first upload auto-named itself `patient-thunder-a29b` when the name field was left at its random default, and it was deleted and re-uploaded before any DNS pointed at it. *Name it before dragging the folder in; after a custom domain is attached, this stops being a two-minute fix.*

**⚠ The deploy folder was staged, and that was not fussiness.** `site/` contains `_exported-bundle.html` — the design tool's rejected 4 MB export, git-ignored and therefore invisible to `git status`, but **not** invisible to a directory upload. Dragging `site/` in would have published a 4 MB indexable page at `/_exported-bundle.html` carrying the design tool's runtime and an origin allowlist. A 24-file folder was staged instead and **the live deployment returns 404 for that path** — verified, not assumed. Any future redeploy must stage the same way; the gitignore will not save it.

**Two settings that look wrong and are not.** (1) **SSL/TLS mode is `Full`, not `Full (strict)`** — correct here, because the origin *is* the Worker running at the edge; there is no Cloudflare-to-origin hop for strict mode to protect. It was checked and deliberately left. (2) **DNS was never edited by hand.** The zone is on Cloudflare nameservers in the same account, so attaching the custom domain wrote the apex record itself — which is also why the MX records survived untouched.

**⚠ And a local resolver lied about the result.** Immediately after `www` was added, `curl` returned `000` with exit 6 (*could not resolve host*) while `nslookup 8.8.8.8` resolved it correctly — the LAN router was holding a negative NXDOMAIN cached from before the record existed, and `ipconfig /flushdns` did not clear it. `curl --resolve www.forgelegacy.app:443:104.21.69.62` returned **200** and settled it. **A local DNS failure is not evidence the deployment failed; hit the edge directly before believing it** — the same shape as this project's standing rule that a green gate on one machine is not a working system.

**Also corrected in this pass:** `Docs/GO-LIVE.md` claimed *"applied state unverified since 0159."* **Wrong — everything through 0162 is applied**: `0155`–`0158` preflighted 15/15 on 08-13, `0159` applied 08-15, `0160`/`0161` evidenced by working squad create and transfer, `0162` confirmed by a route drawn on device 08-16, `0145` applied 08-12, `0144` absent by decision. The v1.0 file was two days stale on its most load-bearing warning.

---

### 0. The landing page exists — and it is an Apple enrollment gate, not marketing (2026-08-16, SITE — no app code, no migration, not an OTA)

**`site/index.html` was the one file left in `site/`, and it is now built**: 152 KB of HTML plus 242 KB of assets, **394 KB against the brief's 900 KB budget**. Rebuilt from `Forge Legacy Landing v5.dc.html` (1,201 lines, Claude Design project `b029488a`) as **plain HTML + CSS**, read alongside `handoff/landing-v5-implementation.md` — the handoff is the better map, the `.dc.html` governs where they disagree.

**⚠ This is not a marketing task with a marketing deadline.** An organization Apple Developer account requires a public, functional website on a domain associated with the organization, and Apple explicitly rejects parking pages and thin sites. The D-U-N-S was requested 2026-08-13 and clears ~2026-08-24. **[SUPERSEDED 2026-08-16 — the DNS blocker described below is closed; see entry 0 above.]** *As written on 08-15: the remaining blocker is DNS, not content — `forgelegacy.app` has MX records only, there is no A or CNAME for the apex, so the page is finished and unreachable.* It was deployed the next day as a **Cloudflare Worker** (not Pages) and the apex now resolves.

**The design tool's own 4 MB export was rejected, again and for the same reason:** its entire body lives inside a JavaScript string, so a search engine or a link preview sees an empty document — for a page whose stated job is *"someone texts you the link"*, that is the whole failure. The rebuild is mostly subtraction: `<x-dc>`, `<helmet>`, `<sc-if>`, `support.js`, `image-slot.js` and the `DCLogic` class are gone; the beat engine is a plain IIFE. **`_ds_bundle.js` and `styles.css` turned out to be unnecessary** — the page carries **zero CSS classes**, so nothing in the component stylesheet was ever reachable. `foundation.css` is inlined as the 41-token subset actually used, hex copied 1:1, **0 unresolved**.

**Two claims on the page were stale and are re-measured.** The FAQ read *"161 database migrations … 2,362 automated tests"* (2026-08-14); it now reads **162 and 2,426**, measured on `feat/route-map`. ⚠ **The migration numbering is not the count** — `0002` was never used and `0152` is used twice, so 162 files span 161 distinct numbers up to `0162`. And ⚠ **the test invocation in `.claude/settings.json` is wrong**: `node --test … src` treats `src` as a *file* and reports one phantom failure; the glob form gives 2,426 passing, 0 failing.

**Three asset questions the `site/README` left open are closed.** (1) **`badge-hall` → `badge-legend`** — `src/domain/rank/thresholds.ts` `FAMILIES` is the authority and says `legend`; the ladder row was always labelled *Legend*, only the filename disagreed. (2) **`bench-demo-frame` is no longer missing** — a still cropped from the shipped render in the public `exercise-media` bucket. ⚠ **It stays a still on purpose**: the animated original is **960 KB, larger than this entire page's budget**, and even subsampled to 14 frames it is 194 KB against a 17.5 KB still. (3) ⚠ **`legacy-bg.png`, `forge-slate.png` and `forge-slate2.png` are BYTE-IDENTICAL in this repo** — so the Legacy hub and the workout slate are the same picture, distinguished only by opacity and overlay. Kept as two files so real slate art can drop in without touching markup. The whole set was re-encoded to WebP: **775 KB of PNG/JPG became 328 KB covering more assets**, plus a generated 1200×630 share card and a favicon.

**⚠ The CTAs are held, per the PO decision of 2026-08-16, and one line of that instruction could not be followed.** Hero and final CTA are non-interactive `<span>`s reading **`Coming to the App Store`** — no `<a href>` anywhere, because a control that looks tappable and is not is worse than one that never invited the tap. The sticky bar is dropped. **The instruction said to keep three `data-analytics` hooks; dropping the sticky bar drops the element that carried `cta-sticky`, so two survive** — `cta-hero` and `cta-final`, both commented in place with the launch swap spelled out.

**Verified by rendering, not by reading** (headless Chromium): page height **11,038px** at 390px against the design's ~10,970 · hero CTA bottom **367px**, inside the first screen · **0 horizontal overflow** at 390px and 1280px · **0 broken images** of 21 · bezel returns at ≥760px · promise grid 4-up, never 3 + 1. `prefers-reduced-motion` collapses all **73** animated elements to end state and stops both scroll loops and the workout sequence; with **JavaScript disabled** all 73 settle and all **1,252 words** stay readable. ⚠ Sampling too early reports phantom broken images and phantom unsettled elements — step down a screen at a time, then wait out the 3,900 ms beat in § 6.

**Two live claims still have expiry dates.** *"Free while we're testing"* and the JSON-LD `offers: price "0"` are both true today and **both become false the moment Phase F of `Docs/Launch-Checklist-Free-And-Premium.md` flips — and nothing on the page will look wrong when they do.** Both are commented at their site. No exercise-count figure appears anywhere, since 721 / 794 / 797 is still unreconciled.
...
**⚠ THE CHECKLIST CALLED IT "FORGE LLC" IN TEN PLACES AND THE ENTITY IS `FORGE LEGACY LLC`.** Not cosmetic: that string is what D&B matches the supporting documents against, what Apple matches the D-U-N-S against, and what becomes the **public seller name on the App Store listing** — Apple accepts no DBA, trade name or branch in its place. Corrected throughout `Docs/Launch-Checklist-Free-And-Premium.md` §9.

**A parent company appeared that no document in this repo had recorded: `Altimealix Holdings LLC` owns Forge Legacy 100%**, same address, and is intended to hold several subsidiaries of which this is the first. **⚠ Utah's Certificate of Organization has no member field**, so that ownership is evidenced nowhere on the stamped certificate — it lives in Utah's principals data and the operating agreement only. Anywhere the structure must be *proved* rather than asserted, that is the gap.

**D-U-N-S is requested and in verification, not pending.** Apple's look-up tool fed the request; **D&B opened case # 10803372 the same day and asked for two documents on a 72-hour clock** — answered 2026-08-15 with the stamped Certificate of Organization, the CP 575 and their Business Information Form (parent declared, 100%). **⏳ The next failure mode is a phone call**: D&B verifies business type and employee count by phone, and an unanswered call is what stalls a case rather than erroring it. Answers must match the form — software publisher · 1 employee · Utah LLC formed Aug 2026 · pre-revenue.

**⛔ The domain is now the critical path, not the D-U-N-S.** Apple's org enrollment needs all three of a D-U-N-S, **a public functional website on a domain the organization owns**, and **a work email on that same domain**. `forgelegacy.expo.app` satisfies neither of the last two — it is Expo's domain. If the number clears in late August against a domain that was never bought, the whole wait was spent and the gate is still shut. That is §9.1b, and it is the item this list already flagged as the one that gets discovered late.

### 0. Background GPS shipped in two builds and never once ran (2026-08-15, CODE — no migration, OTA-deliverable)

**PO's tester ran 6.45 mi of Mt Timpanogos trail in 1:36:17. Forge filed it as 0.01 mi over 1:41:50.** The clock was almost exactly right, which is what made this hard to see and what makes it the same family as #1 below: `elapsedSec` is wall-time-driven, so it ticks straight through an OS suspension and reports a perfect duration for a run the app was not present for. A correct clock is not evidence that anything was measured.

**⚠ `startBackgroundFixes()` GATED ON A PERMISSION iOS DOES NOT OFFER.** The body opened `const { granted } = await requestBackgroundPermissionsAsync(); if (!granted) return false;` — and `granted` is true only for **Always**. iOS's first location prompt offers **Allow Once / Allow While Using App / Don't Allow and nothing else**; Always is an upgrade the OS raises later, if ever. So `startLocationUpdatesAsync` was never reached for any athlete who had not opened Settings by hand, the app was suspended on lock exactly as before `background-task.ts` existed, and the run kept only the few metres of jitter recorded while the screen was still on. **The feature shipped in build 5, was carried into build 6, and did nothing in either.**

**A second defect underneath it: the two permission requests raced.** `start()` fired `clearBackgroundFixes().then(startBackgroundFixes)` and `attachGps()` as parallel `void` calls, so the **Always** request went out while the **When-In-Use** prompt was still on screen. On iOS you cannot be granted Always from `notDetermined` — the OS discards that request silently and **never re-raises it**, costing the install its background tracking permanently. **Confirmed on device:** the When-In-Use prompt appeared, was granted, and the second prompt never came.

**FIXED** — the `granted` gate is gone and the start is attempted regardless (the existing `catch` is the real answer: if the OS refuses, it returns false and degrades to a foreground run exactly as before). "When In Use" is sufficient with `UIBackgroundModes: ["location"]` when updates start in the foreground, which is the case here, and `showsBackgroundLocationIndicator` was already on. The two requests are now sequenced — `await attachGps()` before `await startBackgroundFixes()` — so Always is asked only once an answer to When-In-Use exists.

**⚠ EVERY GATE WAS GREEN THROUGHOUT, AND THAT IS THE REAL FINDING.** `background-wiring.test.mjs` is a **source-string guard**: it proved `UIBackgroundModes` was in `app.json`, that the task was registered at the root, that the buffer drained and that the drain was awaited. It could not see that the function bailed on line 2 and never called the thing all of it existed to serve. Two new guards close it, **both mutation-tested to fail on exactly their own regression and nothing else**: one asserts the `granted` gate has not returned, one asserts `startBackgroundFixes()` is ordered after an awaited `attachGps()`.

**Separately, a correct refusal that read as a broken map.** PO: *"What happened to the Apple map?"* on a 0.22 mi test walk. Nothing had: `TRIM_M` removes 200 m from each end before a route is encoded (D-RTE-1), so a 354 m bout has **no middle** — every point is doorstep — and nothing is stored or drawn. The threshold is the privacy control working. The silence was the defect, since a withheld map and a missing map look identical on screen. `MIN_MAPPABLE_MI` is now exported from `route-privacy.ts` (the same constant `trimEnds` gates on, not a second copy) and the card's caption says **"too short to map"**. Guarded in `route-persistence.test.mjs`. Any real run clears the floor in its first minute; this only ever bites test walks.

**2,420 tests green, tsc clean, lint at baseline (1 error + 13 warnings, all pre-existing).** Committed `1a0bf4b` on `feat/route-map` and pushed.

**✅ SHIPPED OTA 2026-08-16** to branch `production`, iOS runtime `411fd2b68cbe…`, update group `6e416d9f-2624-45a1-8c9d-6e6a15bbda02`. No native config changed, so `eas update` was the carrier rather than a build 7 — and **`fingerprint:compare` was run against build 6 (`078d2838…`) first and returned an exact match**, per the trap in § OTA that once made every OTA undeliverable. It reaches a phone on the next cold start.

**⚠ AND THE PO CANNOT BE THE ONE TO VERIFY IT, WHICH IS THE POINT WORTH KEEPING.** On 2026-08-16 the PO reported a run that tracked with the screen off and drew its map afterwards — on **build 6, which does not contain this fix**: build 6's commit and `app.json` bump are both stamped 11:35 AM on 08-15, and these files were written at 8:03–8:12 PM the same evening. The old `granted` gate therefore passed on that device, and `granted` is true only for **Always** — so the PO's phone already holds the permission this fix exists to stop requiring. **A successful background run on the PO's device is not evidence the fix is unnecessary; it is evidence that his device is the one configuration where the bug is invisible.** The confirming test is a locked-screen walk on **a fresh install that answers the first prompt with "While Using the App" and is never taken into Settings** — that leg has not been run, and running it on an install that has ever been granted Always proves nothing.

### 0. A three-mile run measured 0.0 miles (2026-08-15, CODE + **migration 0162 — ✅ APPLIED, and confirmed by a route drawn on device 2026-08-16**)
...
**✅ MIGRATION `0162_route_and_climb.sql` IS APPLIED** — by the PO on 2026-08-15, through two failures of the file's own self-report (`1b4732a`, `aefd21e`) that made a correctly applied migration read as rejected. **⚠ THIS ENTRY SAID "NOT APPLIED" FOR A DAY AFTER IT WAS APPLIED, AND BOTH FIX COMMITS SAT IN GIT SAYING OTHERWISE.** Corrected 2026-08-16 on device evidence, not on paper: the PO's run drew its route, which cannot happen unless the columns and both function bodies are live. **The dashboard was the last thing holding the stale claim — when git and this file disagree about whether a migration ran, ask the database.** Both function bodies were transformed **by script** from 0151, each substitution asserted to match exactly once — 0151's own header records that rebuilding one from a partial read has silently deleted a shipped feature four times here. **Order against the client does not matter**: a deploy first sends keys nothing reads, the migration first reads keys nothing sends; both store no route and neither errors. **⚠ P-6 Privacy mentions location, route and GPS ZERO times across both of its documents — named as an open gap.**
...
**⚠ POSTGRES CANNOT REVOKE ONE COLUMN FROM A TABLE-LEVEL GRANT — a table grant outranks column grants — and that constraint is the whole bug.** To hide `squads.invite_code` (SQ-D16) and `profiles.training_since`/`training_label` (P-6), `0149` had the only option that works: revoke SELECT on the table, then re-grant every *other* column individually, **enumerated from `information_schema.columns` at the moment it ran**. So every column added afterwards has no grant at all. `0153` added `squads.training_alerts` **one day later**, PostgREST issues `select *`, and **one ungranted column fails the whole STATEMENT, not the column**. A column audit confirmed `training_alerts` was the only casualty and both hidden columns were still correctly hidden.

**⚠ IT WAS PREDICTED IN WRITING AND SHIPPED ANYWAY.** `Docs/Launch-Audit-2026-08-12.md` §4-2: a column-level grant *"must enumerate every remaining column and silently breaks reads when a future column is added."* That is this, verbatim, one day later. **A prediction is not a mechanism** — which is why `0160` installs `reapply_hidden_column_grants()` (0149's loop, callable, with the hidden list in ONE place) rather than re-granting the one column and leaving the trap armed for whoever adds the next one.

**⚠ AND THE PREFLIGHT WAS GREEN THE WHOLE TIME, WHICH IS THE MORE USEFUL FINDING.** It asserted `invite_code` is hidden — true then, true now — while every squad read returned 42501. **A privilege check that only tests what must be ABSENT cannot see what has gone missing by accident.** The inverse check is now a standing preflight row: every non-hidden column on both tables must be selectable. `0160` also asserts **both directions** and refuses to commit if the repair leaked a hidden column — restoring readability by handing back `invite_code` would re-open request-only joining, and `training_since` would publish every athlete's live status to the anon key. That would be a worse bug than the one being fixed. Its event trigger (auto-regrant on any future `ALTER TABLE`) is **best-effort and tolerated on failure**, since a hosted role may lack the privilege; the preflight is the backstop when it is absent.

**Found by going to create a squad for the ACCOUNT-DELETION test** — the bug was nowhere near what either of us was looking at. Gates: `tsc` 0 · **2,362 tests green** · lint at baseline.

### 0. A white screen, and a hook React Native ships that react-native-web does not (2026-08-14, CODE — no migration)

**Reported from the web preview: a blank page on Add Exercise.** The console carried the whole answer — `Uncaught TypeError: (0, b.useAnimatedValue) is not a function`. `CoachChatSheet.tsx` and `HoltMark.tsx` imported **`useAnimatedValue` from `react-native`**, which React Native ships and **react-native-web does not implement at all** (zero occurrences anywhere in the package). On web the import resolves to `undefined`, calling it throws during **render**, and an uncaught throw in render unmounts the tree. Not a broken component — a blank page. The exercise picker mounts the Holt mark, which is why *that* tap was the trigger. Ten call sites, now on the `useState(() => new Animated.Value(x))` form the other ~30 animated values here already use — stable across renders, and unlike `useRef(...).current` it does not trip the react-compiler rule, so the lint baseline is untouched.

**⚠ NOTHING IN THIS REPO'S GATES COULD SEE IT, AND THE REASON IS WORTH KEEPING.** `tsc` is **correct** — the export genuinely exists in React Native's types, so the TYPES are right and the RUNTIME is not. Lint has no opinion, no test renders a component, and **the web bundle BUILT CLEANLY**, because the bundler aliases `react-native` → `react-native-web` at resolve time and never checks that the named binding is actually there. **This is the exact mirror of the `useSafeAreaInsets` crash** that shipped with tsc, 1,420 tests, lint and the web build all green and broke only on DEVICE, because web had a DOM fallback that did not throw. Same class, opposite direction. The lesson recorded then was *"green on web ≠ working on device"*; **the other half is now written down too.**

**THE GUARD IS GENERAL, NOT A BAN LIST.** Banning one name closes this hole and leaves the class open — React Native adds APIs faster than react-native-web adopts them and the next one will look just as innocent. `src/app/__tests__/react-native-web-parity.test.mjs` **reads the parity list out of react-native-web itself** and asserts every value imported from `'react-native'` anywhere in `src/` exists there. If the package adds an API this starts allowing it with no edit; if it drops one, this fails on the code that uses it. **Mutation-verified** — restoring the import to `HoltMark.tsx` fails the guard and names the file. Three deliberate details: it **strips comments first** (both fixed files now document `useAnimatedValue` at length, and a raw scan would fail on the very files recording the fix — this repo has been bitten by that shape before, when a guard read commented-out SQL as present); it **asserts its own parser sees something**, because a parity check that parses nothing passes everything; and it skips `import type`, which is erased before a browser sees it. Found en route: `BaseCard.tsx` imported `ViewStyle` in value position — harmless, since TypeScript erases it, but a type imported that way is indistinguishable from a runtime binding that may not exist on web. Now `import type`.

**⚠ ALSO ESTABLISHED, AND IT IS THE REASON THE CRASH WAS STILL LIVE:** the deployed web bundle was **not HEAD**. Live served `entry-74ff7ab1…` while HEAD builds to `entry-b15086577b…`. Two consequences beyond this crash — the deployed `toProgramState` predates `0155` and so **reads a `finished` program back as Planned and offers to Start a sealed record**, and a deploy that replaces hashed chunks under an open tab produces a **white screen that a reload fixes**, which is very likely the earlier unreproducible one. Gates: `tsc` 0 · **2,362 tests green** · lint at baseline (1 error + 13 warnings, neither new). ⏳ **NOT YET DEPLOYED — the fix reaches nobody until it is.**

### 0. The morning briefing — the first notification in the app that nobody causes (2026-08-14, CODE + migration `0159` — ✅ **APPLIED 2026-08-15** · ⚠ **NOT DEPLOYED — the client toggle is still unshipped**)

**A tester's ask, relayed by the PO:** *"it would be nice to receive a notification in the morning letting him know what his workout for the day was, and maybe a 'let's get it' type of message."* `tsc` **0** · **2,362 tests green** (17 new here; ⚠ the total also carries a parallel session's uncommitted GPS run-tracking work, which was in the tree throughout and moved the count twice while this was being gated — treat the number as "green", not as a measurement of this unit) · lint **1 error + 13 warnings = the documented baseline, nothing added**.

**⚠ IT NEEDED AN AMENDMENT, NOT A TOGGLE — `Docs/Amendments/P-5-Amendment-003-Training-Briefing.md` (LOCKED).** All twenty-one categories in P-5's inventory are **caused by another person**; this is the first caused by a clock. P-5 §1's own audit records that *"any marketing or re-engagement notification"* is absent from every locked document, `Calendar-System-Architecture-v1.0` CAL-D19 says the app *"never notifies about inactivity"*, and H-1 fails when it *"communicates what the athlete has NOT done"*. The amendment grants a bounded exception on one distinction, and every decision in it exists to hold the feature on the near side: **a briefing states what is next; a nudge comments on what you did not do.**

**⚠ THERE IS NO CALENDAR, SO IT NEVER SAYS "TODAY" — and that is the honest version, not a limitation worked around.** `planned_workouts` (0136) has no date column, `ProgramStructure` has no weekday field (`daysPerWeek` is a count), and progress advances only when a `program_sessions` row is written — `progress-core.ts` states outright that elapsed calendar time is forbidden as a judgement input (`Program-Architecture-Amendment-001` §4). The title is **"Next up"**. "Today's workout" would have been wrong the first time anybody trained on a Tuesday instead of a Monday.

**⚠ WHICH CREATES THE ONE REAL DESIGN PROBLEM, AND THE QUIET RULE IS THE ANSWER.** Because the answer is a function of what has been *done* rather than of the date, a daily job returns the **same session every morning** until the athlete trains or skips — right on day one, and by day five a repeated tap on the shoulder about something undone. That is an inactivity notification arrived at *by accident*, which is exactly CAL-D19. So the briefing announces any one open session **at most twice, then goes silent** until progress is recorded, and never says that it went quiet. **The app backs off instead of escalating** — DNA §8 as a mechanism rather than a copy guideline. ⚠ Completion is read **only in order to stop sending**: `last_count >= 2` is a `continue`, never a different body, and a test pins that `last_count` can never reach the wording.

**TONE CAME FREE, FROM A DIAL THE PO HAD ALREADY SPECIFIED.** `profiles.app_prefs.coachIntensity` (CI-D1) exists because of *"Some people want to be pushed… Some people want just reminders. So it's a very big range."* `reminders` gets the facts and no line — CI-D5 and the PO's own example of that level was *a daily helpful coaching reminder*; `drive` gets **"Let's get it."**, the tester's literal phrase, at the loudest register. Experience resolves to `beginner` per **CI-D8** (it is device-local, so the server never has it), so `push` speaks in `plain` — the voice is never louder than the dial has earned.

**DELIVERY WAS ALREADY BUILT.** `pg_cron` + `pg_net` have been draining `push_outbox` every minute since 0120; `profiles.tz` has been populated on every launch since 0099; `program_slots()` (0119) already resolves the next session in SQL. **The only genuinely new SQL is ~25 lines turning `(week_index, day_index)` into a NAME** — and that is a *third* copy of `trainingDays()`'s non-empty-day filter, so it carries 0119's golden-vector discipline: the migration self-checks at apply time, and `briefing.test.mjs` parses those same vectors and runs `scheduleSlots()` over them, so the two walkers are pinned to numbers written once.

**THE THREE GUARDS WERE MUTATION-TESTED, NOT ASSUMED.** Bending an expected vector, adding *"Back at it — you missed Monday."* to `briefing_lines`, and loosening the quiet rule from 2 to 5 each turned the suite red; each was reverted. The copy guard carries **positive controls** (six known-bad lines it must catch) so it cannot rot into a regex that matches nothing — and two wider versions of it were **rejected for crying wolf**: a whole-file scan reddens on the migration's own header quoting the sentence it refuses to send, and a comment-stripped scan reddens on *"has no day object **behind** it"* in a doc string. The literals in the one function that does the wording are the honest boundary.

**⚠ `ecosystem.test.mjs` DID ITS JOB.** It pins the toggle count *"precisely so a row cannot appear without somebody naming where it came from"* — it went red at 12-vs-11 and was updated with the amendment's authority written into it, not silenced.

**⚠ THE COPY WENT 12 → 80 BEFORE ANY OF IT SHIPPED (2026-08-15), AND THE COUNT IS A DESIGN NUMBER.** The seed was six lines per register, which is thin for the one sentence in this feature that carries a voice: `briefing_body` draws on `md5(athlete || item_key || idx)`, so the draw is uniform and independent between sessions and **six lines means a repeat roughly every sixth session** — often enough to read as a canned app, which is the exact failure mode a *"let's get it"* message has. Forty per register puts it near **2.5%**. `plain` coaches the craft (set-up, tempo, range, honest warm-ups, writing the numbers down); `direct` is louder and never harsher. ⚠ **The regex cannot see the rule that actually governs `direct`**, so it is written into the file above the rows: every line is an imperative aimed at **the work** — the bar, the set, the rep, the plate — and never a verdict on the athlete. *"You're better than this"* would pass the guard and still be the wrong product. All 80 clear the shame guard, the no-calendar check, the 120-char constraint and a new per-register duplicate check; `briefing.test.mjs` now pins **40 each**, because the pick is uniform *within* a register and a shortfall on one side would be invisible to every other assertion here and visible only to the athlete holding that dial. The seed is still `on conflict do nothing`, so re-running 0159 over an earlier copy **tops the table up** and leaves a hand-edited line 1–6 alone; the self-check is `< 80`, not `<> 80`, so the next edit to a sentence stays an `UPDATE` rather than becoming a migration. **Full suite re-run at this point: 2,411 tests, 0 failing.**

**Files:** `supabase/migrations/0159_training_briefing.sql` + `supabase/apply/pending-0159.sql` · `src/domain/settings/briefing.ts` (new) · `src/domain/notifications/__tests__/briefing.test.mjs` (new) · `notifications.ts` · `destination.ts` · `settings-live.ts` · `src/app/notifications.tsx` · `push.test.mjs` (repointed — 0153 still owns the union, 0159 now owns the two preference functions, so `fnBody` takes its source).

**✅ APPLIED 2026-08-15, WITH THE 80-LINE COPY IN THE SAME PASTE** — confirmed empirically in the SQL editor, not inferred from a clean run: `briefing_lines` reads **plain 40 / direct 40**, and `push_pref_default('squad_training')` still reads **true**, which is the assertion that matters most in this file. 0159 restates `push_pref_key` and `push_pref_default` from 0153's bodies, and it was applied *after* 0160–0162 had already landed — so it was checked first that nothing in 0155–0162 redefines either function or `notification_events_for`. Nothing does; 0159 is simply their newest definer. `briefing_schedule` needs no explicit grant for the same reason `week_templates` (0157) and `custom_exercises` (0128) need none. Applying it sent **nothing** to anybody: `training_briefing` defaults OFF and `briefing_schedule` was created empty.

**⚠ STILL NOT DEPLOYED, AND THAT IS THE WHOLE REMAINING GAP.** The database can send a briefing; no athlete can turn one on, because the toggle and the day/hour picker live in JS that has not shipped. This is the 0153 pattern exactly — *a migration lands and nothing appears in the app* — so it should not be read as a broken migration when nothing arrives. Client side is JS-only, so it ships by **OTA**; `expo-notifications` is already in the build. **Do not publish from this tree until the route-map / GPS run-tracking work is committed or stashed** — publishing bundles the working tree.

### 0. Coach Holt, from a real session — six reports, and two of them were the engine being right (2026-08-14, CODE only · no migrations · ⚠ NOT DEPLOYED)

**PO feedback after using the v2 chat.** `tsc` 0 · **2,285 tests green** · lint at the documented baseline. Commits `576e6ba` · `8159e8f` · `d7f3ca6` · `14f1baa` · `c343bce`.

**⚠ "A HOME BODYWEIGHT WORKOUT CAME UP WITH ONE THING." Two independent causes, and the second was much larger than the report.**

*(a)* **The chat never read the athlete's Home Gym.** `equipmentForEnvironment('home', owned)` returns `owned` verbatim and the sheet never set it, so **"My home gym" resolved to an empty list — byte-identical to "Bodyweight only"**. An athlete with a rack in their garage was handed push-ups. The dead wizard at `/coach` has read `fetchHomeGym` since it was written; the chat simply never did. `null` still means never-set-up and does not become "I own nothing".

*(b)* **Two movements is not a session, and the guard only caught zero.** Swept across 13,200 combinations: **2,240 return one or two movements** and 1,040 return none. A bodyweight *pull* day came back as a single Plank; a beginner with a bad lower back got *"Walking Lunge, Dead Bug"*. **The engine was not wrong about the catalogue** — there is not a single vertical or horizontal pull in 733 exercises that needs no equipment at all (0 of 31, 0 of 57), now asserted so the refusal fails loudly if that ever changes. It was wrong to hand back the remainder as though it were a workout. **The threshold is measured, not chosen:** at a full gym exactly 20 of 2,640 fall under three, and all 20 are the same case — a shoulders focus from someone who said their shoulders are the problem. `whyThin` RE-RUNS the builder to find which lever would have fixed it rather than inferring it, because an empty home gym AND a stated limitation is a real combination. ⚠ And he never offers to train through it.

**THE FOCUS PILLS ARE THE PARTS NOW, AND YOU PICK SEVERAL.** The list shipped the COMBINATIONS — "Chest & Triceps", "Back & Biceps", "Shoulders & arms" — three pairings we happened to think of, with no way to ask for a fourth. Twelve atoms instead, multi-select, with a deliberate Build. A split is exclusive and parts combine: "Push" already contains a chest movement, so merging it with Chest would spend a third of a fixed budget pressing. **⚠ Cardio is not a muscle** — it rides as a flag rather than a ninth `BodyPart`, and it gets a slot RESERVED: letting it compete on merit produced "Back & Cardio" byte-identical to "Back", under a title that said Cardio. The title is now named from what is in the day.

**CHOOSE HOW LONG THE BLOCK RUNS** — One week · 4 · 8 · 12. ⚠ It moved out of the opener and into the questionnaire, **after the goal**, and the move is the fix: a race's length is counted back from `raceDate` against 6–12 week floors, so "12 weeks" then "Run a marathon" would have been silently overruled. Asked after the goal, a race is simply not asked. ⚠ A bare typed `"1"` was **twelve weeks** — `interpret` matches chip labels by containment and "1" is in "12 weeks" — now snapped to the nearest rung ahead of the loop.

**⚠ AND THE 13,440-CASE CROSS-PRODUCT HAD SILENTLY STOPPED BUILDING ANYTHING.** Adding a question made `readyToBuild` false for every athlete in `everyAthlete()`, so `chat-build-path.test.mjs` returned "asked" 13,440 times, called `assemble` zero times — and stayed green, because nothing threw. The only visible symptom was the suite getting faster. `built === ran` is now asserted so the next question added there fails loudly instead of quietly emptying the file.

**OPEN A WEEK AND SEE THE SESSIONS IN IT.** The days were not missing from the UI, they were missing from the CARD: each row carried the string `"4 sessions"`, a count composed for display, with no data behind it to open onto. Read per week through the same `plannedDays` the scheduler uses — a block that runs six days then five shows the week it really is.

**IN-WORKOUT HOLT.** ⚠ *"After I do the first set it doesn't go away."* This reverses a decision recorded in `coach-says.ts` (*"a technique cue is as true on the fourth set as the first"* — true, and beside the point). Both `progression` and `planCue` are what he says as you WALK UP to the bar; after a set they are words that will not leave. Retired on **sets logged**, not `heaviestThisSession`, which is null for every plank and carry. `live` is exempt — it is written BY a completed set. And the coin now speaks the chat's language: the `HOLT` eyebrow on every line, `--fl-surface-elevated`, 15/21.5 cream, the chat's own spacing.

**Also:** "Get fitter" is no longer offered (the goal keeps its skeletons — deleting it would break a block somebody is six weeks into); "Build a program" became **"A program or a week"**; "Ask Holt something" became **"How do I…"** and the pinned bottom row was removed.

### 0. Workout Entry — the card in a void became a screen (2026-08-14, CODE only · no migration · ⚠ NOT DEPLOYED)

**Built to `Forge Workout Entry.dc.html`** (design system `ForgeLegacyVisualFoundation_5368b2`), replacing the Freestyle and Resume states in `workout.tsx`. `tsc` 0 · **2,296 tests green** (11 new) · lint at the documented baseline · clean web export.

Both were a `Card variant="hero"` floating in a vertically-centred void with left-aligned type inside. The bordered container is deleted: the workout identity sits directly on the stone as one bottom-anchored centred column, resolver-driven artwork carries the top half, and **bronze is spent on exactly one element — the CTA**. The dismiss went from bronze to warm gray, which is what makes that true. New `WorkoutEntry` component; the two states share the shell and differ in six props, not a `variant`.

**⚠ THE ARTWORK GRADING IS NOT THE SPEC'S NUMBERS, AND THE SPEC'S OWN RULE IS WHY.** §2 asks for `opacity: 0.72` + `mix-blend-mode: screen` + a vertical `mask-image`, and states the reason: *"deliberately identical to the Home hero card's grading… Same asset, same grading, everywhere."* Those numbers grade an **opaque** asset — the blend and mask exist to drop a rectangular ground. This project cut those 72 masters to real alpha in a later pass, which is why `TodaysWorkoutCard` draws them at opacity 1 with no blend, mask, fade or crop. Following the numbers would have made this screen *fainter than Home*, i.e. broken the rule it cites. **Matched the shipped Home card instead**, and neither half of the mask is missed: the top fade hid a rectangle edge that no longer exists, and the bottom fade is performed by the asset's own alpha.

**A gradient scrim was considered as the mask substitute and rejected as worse than nothing** — its opaque end lands mid-screen on a *textured* slate, flattening it into a solid block with a visible horizontal seam. A real mask needs `@react-native-masked-view`, and **a new native dependency moves the build fingerprint, which costs every athlete an OTA** — for a fade the asset already performs.

**⚠ THERE IS A THIRD STATE THE DESIGN DOES NOT DRAW.** `pendingLaunch` means the athlete picked a *different* workout while one was still open, so its dismiss does not end anything — it starts the other one. §9 makes Resume's dismiss "End workout"; labelling that case the same would hide the more important half, so it keeps its shipped copy. Everything else follows §9 exactly.

**The Resume body is session state now, not a question.** `N sets logged. Resume where you left off?` asked what the two buttons already ask; it is `4 exercises · 11 sets · 38 min` over `Last: <exercise>` — the entire progress treatment, no bar and no ring. New pure `entry-summary.ts` behind it, because all three numbers are decisions: **"sets" means sets LOGGED** (16-planned-3-done can honestly be called either, and the resume prompt exists to say how much work is sitting here), the last lift is the last one **worked** rather than the last in the list, and **the minutes are wall-clock from `startedAt` even when that reads `14h 6m`** — `save.ts` stores exactly that subtraction, so a flattering number would make the screen disagree with the history it is about to write.

**`Date.now()` cannot be called in a render body** — the react-compiler lint rejects it outright, and it would have jittered the figure on every unrelated re-render. Frozen at the moment the prompt is raised, which is also the moment it describes.

**`Button` gained `size="lg"`** (64dp · 17px · 2.4 tracking) rather than the screen hand-rolling `flGradient.bronzeFill` — which §7 itself forbids. **And the 46px bottom padding is measured on a canvas with no home indicator**, so the safe-area inset is *added* to it; shipping the constant would have put the dismiss under the indicator, which is the defect the coach sheets shipped with.

### 0. The builder's footer was pointing at the exit (2026-08-14, CODE only · no migration · ⚠ NOT DEPLOYED)

**PO:** *"When I am building a week template, or a program, it needs to be more obvious that I can save that day and move on to the next. Right now it has 'save workout' as the more obvious button, and then 'save and move on to day B' underneath, and that should be reversed. All the way until the last day and week."* `tsc` 0 · **2,285 tests green** (7 new here; the total also carries a parallel Coach session's work) · lint at the documented baseline · the cross-week branch **mutation-tested red**.

**The hierarchy was backwards against what building actually is.** Nobody opens a four-day program to author one day — the sequence *is* the task. `Save Workout` (which returns to the overview) was the bronze primary; `Save & go to Day B` (which continues the work) was a grey line of text underneath it. Reversed: forward leads, leaving is the quiet link, and on the genuinely last day the two swap back so Save is the only thing on offer.

**⚠ AND "FORWARD" DIED AT THE END OF EVERY WEEK.** The footer only ever knew about the next day *within the open week*, so in a twelve-week Customize-per-week program the forward path ran out at Week 1 · Day D and the athlete was dropped back to a list to find Week 2 themselves — eleven times over. New `nextDayStop` in `program-draft-model.ts` answers the whole question: the next day, else **the first day of the next week**, else null. A test walks a 3-week × 4-day draft by pressing forward and asserts all eleven moves are reachable.

**⚠ It must NOT read `weeks` in repeat-template mode.** `vary: false` with `weeks: 8` is the normal shape — `d.days` *is* every week — so stepping into "week 2" there would invent seven weeks of days that do not exist. Guarded, and tested from that side too.

**Walking into a week is not the same as opening a day in it.** An untouched week with a built sibling gets offered a copy, and that offer is the difference between rebuilding twelve weeks by hand and copying one. Crossing over from Week 1 · Day D raises the same copy sheet over the newly-opened day, so neither of its answers (copy, or start empty) loses the move.

**The day name is truncated in the button and not in the accessibility label** — a name takes 30 characters, and `Save & go to Week 2 · Upper Body Heavy Push` wraps a footer onto three lines. A truncation is a fit problem, not something to read aloud.

### 0. "Which squad?" took one answer and there were three (2026-08-14, CODE only · no migration · ⚠ NOT DEPLOYED)

**PO:** *"When I go to share my workout or an honor or something… if I click squads then I can only pick one. I want to be able to easily select 1/3, 2/3, or 3/3 of those."* Every squad picker in the app was a **list of buttons**: the tap posted and closed the question. Sharing one session to two squads meant walking the whole flow twice, to three squads three times — three separate posts, with nothing on screen saying that was what you were doing. `tsc` 0 · **2,268 tests green** (10 new) · lint at the documented baseline (1 error + 13 warnings, none new).

**Three surfaces, one rule.** `ShareSessionSheet` (workout — Complete + Activity Detail), `compositions/ShareSheet` (every ceremony: rank-up, honor, goal, program graduation) and `share-config` (Share Transformation) each asked the question in its own shape. They now share `src/domain/share/fanout.ts` — pure, `node --test`-able, no alias imports — plus a `SquadSelectList` component for the two list-shaped pickers. The ceremony sheet keeps its chips (they were a radio group; each one toggles now) with a `Select all` above them.

**⚠ THE FAN-OUT IS NOT ONE ROW PER SQUAD WHEN FRIENDS IS INCLUDED, AND THE NAIVE VERSION INSERTS CLEANLY.** 0074 states the audience rule as an equivalence — `check ((audience = 'FRIENDS') = (squad_id is null))` — so a `BOTH` row must name exactly one squad. Emitting three `BOTH` rows for three squads passes every constraint and is still wrong: `friends_feed` selects `FRIENDS`+`BOTH`, so the same workout would appear in every friend's feed **three times**. The shape that is right in both feeds is asymmetric — **one `BOTH` row carrying the first squad, then a plain `SQUAD` row for each of the rest**. `squad_feed` filters on `squad_id` alone, so every chosen squad still sees it once. A test asserts the equivalence holds for every row the function can emit, and a second asserts exactly one friends-visible row for any squad count.

**Inserts are sequential, not `Promise.all`.** A partial failure has to be attributable: the toast names what actually landed (`Shared to Alpha and Bravo`) rather than implying the whole share failed and inviting a duplicate re-post.

**One squad is still one tap.** The picker only appears when there is a choice to make; a single squad posts straight through, exactly as before. `Select all` is the 3-of-3 case in one tap, and toggles back to `Clear` so the extremes are always one tap apart.
...
**⚠ THE FIRST FINDING WAS THAT THE RANK RULE DID NOT EXIST.** Graduation is `logged >= program_total_sessions(structure)` (0104); rank counts rows with `state='graduated'`; `honor_metrics()` counts the same rows. **None of the three has ever read program length.** "Only 4-week programs count" was true for exactly one reason: `WEEKS_MIN = 4` in the Builder's draft clamp, whose only authority is a line in a design HTML. A product rule holding on a UI stepper, with nothing behind it — so opening length to one week would have let a deload week buy a rank family and five never-revocable honors. **The rule is now written down** (`Rank-Computation-Model-Amendment-003`, **D-RCM-30**) and enforced where the athlete cannot reach it.

**A FIFTH PROGRAM STATE, AND IT IS THE CHEAP ANSWER.** A sub-4-week program seals as **`finished`**, not `graduated`. Because rank (`rank-live.ts:51`) and honors (`honor_metrics`) both filter `state = 'graduated'`, **neither needed a single change** — the alternative was hand-rebuilding the ~200-line `honor_metrics()` to add a jsonb predicate, which is precisely the operation that has silently deleted a shipped branch in this schema four times. It also keeps `viewForState` and `shelvePrograms` pure functions of *state*. Named `finished` rather than `completed` because `SessionState` already uses `'completed'` for a single session's mark and both are in scope in `program/[id].tsx`, where `x === 'completed'` compiles against either.

**⚠ BOTH COMPLETION DOORS ARE GATED.** A program's last session can be **logged** *or* **skipped**. Gating only `save_workout` would have left a one-week block skippable to a full graduation — five permanent honors for four taps and no training. `skip_program_session` carries the same verdict (D-RCM-30 R3). Both bodies were **machine-generated by transform** from the shipped 0151/0123 originals with every edit asserted, and a `node --test` guard now checks the installed migration still contains `floors`, `incline_pct`, `group_kind`, `notes`, `program_slots` and `evaluate_honors` — the branches this repo keeps losing.

**THE FEATURE.** `week_templates` (0157) holds a `ProgramStructure` **pinned to one week by a CHECK constraint**, so every scheduler, slot walker and builder surface works on it unchanged and Start is literally `createProgram()` + `start_program()`. Authoring is a **mode of the existing Program Builder** (`?mode=week`) rather than a new screen, because `SetupView`/`DayBuilder`/`TemplateDaySheet` are all local to that 2,845-line file. New: `/week-template/[id]` (**W-29**), a "Your Weeks" shelf on the Templates hub, and a `+` chooser. **⚠ The draft slot was a single global AsyncStorage key** — opening the week builder would have silently destroyed an in-progress 12-week program draft; it is now keyed by kind.

**MONEY.** `caps.programs` is 3 **lifetime and never reopens**, so three throwaway deload weeks would have permanently closed the door on a real block. New `short_programs` allowance (`Monetization-Architecture-Amendment-004`, 3 free / unlimited paid), split on exactly the line D-RCM-30 draws. **Starting a week you already saved charges nothing more** — the double-charge would have fired a cap of 3 at 2.

**⚠ THE ONE THAT COULD SHIP THIS DARK.** `entitlement_config` has exactly one row, and **changing a column DEFAULT does not touch it**. Without 0158's explicit `UPDATE`, `my_entitlement()` returns no `short_programs`, the client coerces missing to 0, and **every athlete including Premium is blocked from the whole feature** while every gate reports green. 0158 asserts the row back at apply time; verify §6a before shipping the client.

**Also fixed, reported by the PO mid-session:** *"I clicked the build a template button in the workouts tab and it took me to the same place the start workout would."* True — the create row under **Your Templates** opened the *Start Strength* chooser. `W25-Amendment-001` had routed six doors there and that was right for five; this one is an **authoring** door, and the sixth on its own list (the Templates hub's "New") already went straight to the builder. Now it does too (**W25-A1-D8**). The guided tour's `wk-templates` step still taught the pre-W-25 world and is corrected.

**Holt authors short blocks coherently rather than by truncation.** `prescribeReps` ramps from the *bottom* of the rep range in week 1 — so a 1-week block would have shipped the introductory week of a mesocycle that does not exist. At ≤3 weeks it now anchors mid-range and does not ramp (`PAS-Amendment-007`, **PAS-A7-D2**). Endurance keeps its own higher floors, untouched.

**Docs (8):** `Program-Architecture-Amendment-002` · `Rank-Computation-Model-Amendment-003` · `M-4-Amendment-001` · `W4-Amendment-001` · `Program-Authoring-Standard-Amendment-007` · `Monetization-Architecture-Amendment-004` · `W26-Amendment-002` · `Week-Template-Detail-Spec-W29`. **D-RCM-30 §3 confronts the parity gap it creates** — programs credit at 4 weeks, self-directed blocks still need 6, and D-RCM-29 calls them "additive at parity" — and accepts it *in writing* rather than leaving an athlete to find it.

**Gates:** `tsc` 0 · **2,207 tests green** (+38 new, incl. a mutation-proven Holt guard) · lint at the documented baseline (1 error + 13 warnings, none new). ⏳ **Not applied, not deployed.** Apply strictly **0155 → let it commit → 0156 → 0157 → 0158**: 0155 adds enum labels and a label cannot be *used* in the transaction that adds it (`55P04`).

### 0. The weekly review would not go away, and only one of the three reasons was a bug (2026-08-13, CODE + amendment — migration `0152` STILL NOT APPLIED, bundle now written)

**PO: *"I've clicked preview on it and also I've clicked skip and it keeps popping up."*** Three independent causes behind one sentence, and the honest finding is that the fix shipped on 2026-08-12 had never actually run.

**⚠ THE 24-HOUR WINDOW HAS NEVER ONCE BEEN ENFORCED.** `0152_weekly_review_created_at.sql` was authored, gated, written up on this board — and never applied. So `ensure_weekly_review()` does not return `created_at`, `reviewWindowOpen()` reads null, and **null is deliberately OPEN**: the safe failure that stops an unapplied migration from vanishing every athlete's review. That safety is exactly what hid the problem. The card lived the full seven days, which is the behaviour 0152 exists to end, and the board has been recording it as fixed since the 12th. **This is the "amendment locked but never applied" pattern reaching a shipped surface the PO looks at daily.** Paste-ready bundle now exists: `supabase/apply/pending-0152-weekly-review.sql` (verbatim body, diff-verified against the migration; **⚠ two files are numbered 0152 — check both**).

**⚠ SKIP LASTED AS LONG AS THE HOME SCREEN STAYED MOUNTED.** A `useState(false)`, on the recorded reasoning that *"'not now' expiring on the next launch is the honest reading of it."* The reasoning fails on its own terms: the PO tests the **web preview**, where "the next launch" is a page refresh — the dismissal expired before they had left the screen. Now stored device-locally keyed to `week_start` (`weekly-review-seen.ts` + `-model.ts`, the `podium-seen.ts` pattern), cleared on account switch by `first-run.ts`. Still this week only, still not a preference, still deletes nothing — `/weekly-review/[week]` opens a retired review forever.

**AND READING THE REVIEW DISMISSED NOTHING AT ALL.** `onView` only navigated, so the athlete opened their week, read it, came back, and the card was still there asking to be opened — the app failing to notice what it had just watched them do. **View now retires the card too.** That one was missing rather than mis-tuned.

**⚠ THE TWO SAFE FAILURES DELIBERATELY POINT OPPOSITE WAYS.** Unreadable storage reads as *nothing retired*; an absent `created_at` reads as *window open*. Both err toward the athlete seeing their week: a review shown twice is an annoyance, a review silently swallowed is the feature not existing. Both directions are asserted in the tests. Also latched: the RPC and the local read now settle together via `Promise.all`, so a card the athlete retired last night cannot paint for a frame while the network answers.

**`Docs/Weekly-Review-Design-Brief-v1.0.md` §3 and §5 said the opposite** and are amended, not left to rot — `Docs/Amendments/Weekly-Review-Amendment-001-Card-Retirement.md`, with both superseded lines marked in the brief itself.

**Gates:** `tsc` 0 · **2,207 tests green** (9 new) · lint at the documented baseline (1 error + 13 warnings, none in the touched files). ⏳ Not deployed — the tree still carries a parallel session's uncommitted work.

### 0. A run typed in kilometres was stored in miles (2026-08-13, CODE — no migration)

**`log-activity.tsx` labelled its field "Miles" unconditionally and passed the typed figure straight through as `distanceMi`.** A metric athlete who meant 5 km typed 5 and a **5-MILE** run was stored — not a rounding error but a run 61% longer than the one they did, feeding distance goals and auto-tracking with it. `units.ts` forbids exactly this by name, and the fix required nothing new: `useUnits`, `distanceUnitFor` and `fromDistanceIn` all existed, all had tests, and **the screen used none of them.**

**⚠ THE INTERESTING PART IS WHY EVERY GATE WAS GREEN, AND IT IS A NEW SHAPE FOR THIS BOARD.** The helpers were correct and unit-tested; the composition was never tested because it lived in a `.tsx` screen `node --test` cannot load. **A tested part is not a tested path.** So `CARDIO_OF_DISTANCE_ACTIVITY` moved into `domain/workout/conditioning.ts` — the pure module — and the five new tests assert the chain a logged activity actually travels (typed figure → activity → unit → canonical miles) rather than the pieces. Two of them **fail against the old behaviour**, verified by mutating `distanceUnitFor` to return `'mi'` unconditionally and confirming the failures before reverting. The type import into `conditioning.ts` is `import type` deliberately: erased before `--experimental-strip-types` runs, so it creates no runtime cycle with `save.ts`, which imports from it.

**It was invisible because `0139` put every athlete on imperial**, where `fromDistanceIn(x, 'mi') === x` exactly. It was never triggered; it was waiting for the first person to choose km — which, with 20 testers about to arrive, is now a live risk rather than a theoretical one.

**⚠ AND SWIMMING WAS WORSE THAN METRIC.** Nobody writes a pool session in miles. `distanceUnitFor` gives the pool yards (metres on metric) — the scale `CardioBlockCard` and the program builder already use — so "1000" now means 1000 yd rather than an unauthorable 0.568 mi. **Switching activity now CLEARS the field** when the unit changes: keeping "3" across Run→Swim would turn a 3-mile run into a 3-yard swim, the relabel-without-converting `units.ts` forbids, and converting it would be worse (nobody types 3 for a run and means 5280 yd). Duration and notes are unit-free and are kept.

**Gates:** `tsc` 0 · **2,213 tests green** · lint at baseline (1 error + 13 warnings, both pre-existing). ⚠ **Still open from the launch audit: ~34 of 48 weight/distance surfaces do not convert.** This closed the one on the athlete's *input* path, which is the only class that corrupts stored data rather than a display.

### 0. The release is two stages, and the long pole is paperwork nobody has started (2026-08-13, DOCS only — no code, no migration)

**PO decisions: the 20 testers get it now and some of them get it free · the public release carries the paywall · the public release ships under a NEW Forge LLC organization account.** `Docs/Launch-Checklist-Free-And-Premium.md` → **v1.1**, extended rather than forked — this repo's recurring failure is a second document that competes with the first. Three new sections: **§8 Stage 1 (TestFlight, existing team `G722GV8H8C`)**, **§9 Apple entity migration**, **§10 App Store listing artifacts**. Execution order is no longer section order: **§8 → (§9.1 ∥ §4) → §10 → §5 → §6 → §7.**

**⚠ THE SCHEDULING FINDING, AND IT IS THE ONE THAT MATTERS.** An organization Apple Developer account requires a **D-U-N-S number** matched to the legal entity — **3–14 days, cannot start before the LLC exists, and has not been started.** Phase E (P-8 + RevenueCat + 6 SKUs + sandbox verify) is roughly the same length and has **no dependency on it**. Run in series that is two waits; run in parallel it is one. **Nothing about Stage 1 waits on any of it** — `default_tier` ships `PREMIUM`, so every tester resolves entitled and no cap is felt, which *is* the free access the testers were promised. **No paywall code is needed to put the app in their hands.**

**✅ RESOLVED SAME DAY — THE PREFLIGHT RAN AND THE DATABASE IS CLEAN THROUGH `0154`. All 24 checks green; `0144` correctly MISSING by decision.** So `0150` was never actually broken in production, `0151` was already in before the code that needs it deployed, and `0148` is present. **Every source was wrong in the SAME direction — describing as pending work that was already done.** That is the mirror image of this project's recorded failure (a migration that lies about having worked) and it is not harmless: re-pasting an applied migration is not free, and `0141` has already died on `42P13` when re-run. **Stage 1's remaining work is a device test of account deletion, the units ruling, and a build — no SQL at all.** The paragraph below is kept as written, because the reasoning for running the preflight FIRST is exactly what made this cheap.

**⚠ AND STAGE 1 IS BLOCKED ON A DATABASE THAT NOBODY CAN CURRENTLY DESCRIBE.** This board, the checklist's own standing rule ("applied through **0143**", eleven files stale) and the working notes **disagree with each other about 0145, 0150 and 0152**. §8.1 is therefore preflight-before-anything: `supabase/apply/preflight-0146-0153.sql`, and **record the answer here in the same pass** — an un-recorded preflight is a session that gets spent twice. Also caught while writing it: **`0152` is used twice** (`0152_discover_trained_today.sql` and `0152_weekly_review_created_at.sql`), so that migration cannot be identified by number alone.

**Three submission blockers now named as blockers rather than as bugs.** **`0150`** — `0147` revoked EXECUTE on `evaluate_honors` from `authenticated` and its three callers are `security invoker`, so Finish Workout, Continue Training and session-skip are dead for every athlete and the workout rolls back *lost*; no build reaches a tester before it lands. **`0151`** — must precede deploying current code, because three data modules select `floors` and PostgREST's `42703` becomes *"there is nothing to continue"* (the 0117/0118 failure exactly). **`0148`** — **App Store Review 5.1.1(v) requires in-app account deletion**; the client exists (`account-live.ts`, `account-settings.tsx`), the round trip is unverified.

**§9.3 is the item most likely to bite, and it is recorded as a question for Apple rather than an answer.** Bundle IDs are globally unique and `com.qest4.forgelegacy` belongs to the qest4 team. Release-and-re-register kills the TestFlight builds under it; a new bundle ID is *a different app* to iOS, so tester installs sit alongside rather than upgrade; and an Apple app transfer preserves everything but generally requires a prior public release — which contradicts "the first public release is under the LLC." Whichever wins, the `forgelegacy` scheme and the Supabase redirect allow-list get re-checked, and **§9.5: push is configured against the old team's APNs key and stops silently under a new one.**

**§10 is entirely unstarted — no store-listing doc existed.** Hosted privacy/terms URLs · **the privacy policy's "exhaustive" collection list omits usage analytics (`content.ts:53`) and must be fixed BEFORE the App Privacy labels are signed**, since a declaration contradicting your own posted policy is the worst version of that bug · support URL · screenshots/description/keywords/age rating · **a seeded reviewer account**, because the social pillar is unreviewable from an empty one and "no content" reads as Guideline 2.1. **§6.2 superseded by §9.6** — Small Business Program enrollment is per-entity, so it belongs to Forge LLC.

**Still open, both touching shipped surfaces (Launch Audit §4):** the **distance-units doctrine conflict** (`units.ts:8-10` forbids converting distance, `0139:10-14` makes `units` the single switch, and the code obeys both in different files) — whichever is chosen leaves the other document wrong and wanting a superseded banner; and whether the **storage-orphan ledger** extends to personal photos or dashboard cleanup is accepted for launch. **Said out loud: iOS only. There are zero Android builds.**

### 0. A field you cannot type into, in fourteen places (2026-08-13, CODE — no migration)

**PO: *"When I go to log a run it's not letting me put in the time."*** Two unrelated faults behind one sentence, and then a third that was the general form of the second.

**⚠ THE DURATION FIELDS ON `/log-activity` HAD COLLAPSED TO A SLIVER OF BORDER.** `InputField` takes no style prop, so its root `<View>` is unstyled — invisible in a COLUMN parent, which is why the Miles field directly above worked. In the ROW parent `durationCell` an unstyled View is sized by its content, and its content is a `TextInput` carrying `flex: 1, minWidth: 0`, which contributes **no intrinsic width in Yoga**. Nothing was disabled and nothing threw; there was simply nowhere to put a caret. Fixed at the call site rather than by widening a Tier-2 composite used on eleven screens. **The only row-parent `InputField` in the app — every other one is in a column and stretches.**

**⚠ AND THE CARDIO TIME FIELD RAISED NO KEYBOARD ON AN IPHONE, WHICH THIS REPO ALREADY KNEW.** A browser opens the software keyboard only for a `focus()` issued INSIDE the user-gesture call stack; RN's `autoFocus` fires when the element MOUNTS, one commit after the tap. `CardioBlockCard` swapped a `<Text>` for a `<TextInput autoFocus>` on press — and `SetGoalPanel`'s header **names that file** as the one still carrying the broken pattern. The defect was written down in a comment for a fortnight instead of being fixed. Its six typeable fields are now always-mounted inputs, so the tap lands on something that already exists.

**⭐ AUDITING FOR "EVERYTHING WITH THE SAME THING TO IT" FOUND TWELVE MORE.** Every `BottomSheet` is a bare `<Modal visible={open}>`, so **its children do not exist at the moment of the tap** — every `autoFocus` inside one has the bug. `workout.tsx` had discovered this, solved it locally with an invisible primer input, and left **two of its own overlays unfixed three hundred lines from the comment explaining it** (its primer hardcodes `decimal-pad`, right for a weight and wrong for a name). New `KeyboardPrimer` provider: three permanently-mounted offscreen inputs, one per keyboard, focused synchronously inside the gesture — three because `keyboardType` is a state write and a state write does not land before the `focus()` on the next line. Wired into all twelve. Also recorded: `ForgeSearchHeader`'s synchronous `focus()` is a **guaranteed no-op** — its `inputRef` is only attached in the branch that is not mounted when the Pressable holding it renders. It has no consumers today.

**Holt stops repeating advice you have already outrun.** PO: *"I did one set of 85lbs for ten reps, coach holt said move up to 95lbs... the second set I actually did 165lbs. He still said move up to 95lbs. He needs to stay current."* Two lines could go stale and **fixing either alone would have looked like fixing nothing** — they say the same thing here (a 10 lb step off 85 is 95 either way) and `live` outranks `progression`, so clearing the nudge just uncovers an identical sentence. One rule for both: a line telling you to reach a weight is spent once you have logged a set at or above it. `hold` and `back_off` are exempt by design — they name a weight to WORK AT, and retiring them on the set they asked for would silence him mid-exercise; bodyweight is exempt because `0 >= 0` would kill every push-up line on the first set.

**⭐ AND TWO TEST GUARDS WERE WEAKER THAN THE THINGS THEY GUARD, both found by mutation testing rather than by anything failing.** `push.test.mjs` matched source text, so **a branch commented out still read as present** — wrapping branch 14 in `/* */` kept the sixteen-branch assertion green, which is the most literal form of the fault it exists to catch. And the new keyboard guard's first version asked only *"does this file mention the primer"*: deleting the prime from `workout-complete`'s rename sheet left the one in its template-name sheet, and the test stayed green over a field that had gone back to being untypeable. Both now strip comments and count occurrences; twelve mutations across the two go red at the assertion that names them.

**Gates:** `tsc` 0 · **2,169 tests green** · lint at the documented baseline (1 error + 13 warnings, none in the touched files). ⏳ Not deployed — the tree carries a parallel session's uncommitted work and publishing bundles the working tree.

### 0. Your squad knows when you train — and the honor card's WHY reaches the one honor it was missing (2026-08-13, CODE + migration `0153` — ✅ **APPLIED 2026-08-13 by the PO**)

**PO: *"Squad leader should be able to set if they want a notification for when someone in the squad starts working out. And then individually you can set whether or not you want those notifications. Either when that squad member starts or finishes."*** Every piece of this already existed and none of them were connected. `profiles.training_since` has been written the instant a session begins since **0086** and **nothing has ever triggered off it** — the database has known who was training for thirty migrations and only ever answered when a screen asked. `public.workouts` had **never carried a trigger of any kind**, so the only way a finished session reached a squad-mate was the athlete deliberately sharing it to the feed.

`0153` adds branches **15 and 16** to `notification_events_for` (`squad_training_started` / `squad_training_finished`), the first trigger ever placed on `workouts`, and one on the presence column. **THREE SWITCHES, AND THEY ARE THREE DIFFERENT DECISIONS:** `squads.training_alerts` is the LEADER's, per squad; `squad_members.notify_start` / `notify_finish` are the MEMBER's own, per squad, split because "tell me so I can join" and "tell me they're done" are different wants. **All three default to off, so applying this to the live database sends zero notifications to anybody.** The P-5 row that governs push (`squad_training`) is the one ambient default in that file that is **ON**, deliberately: nothing can reach it until two people have already opted in on two other screens, so defaulting it off would make those deliberate acts produce silence.

**⚠ AND THE FOURTH GATE IS NOT A SWITCH.** Both branches carry `vis_clears(visibility->>'training', 'squad')` — the ACTOR's own audience, the same predicate `training_now()` has used since 0086. Somebody who set their training to private is invisible on Live Now today, and a squad-mate's preference must not become the back door around it. The person being announced never opted in to being announced.

**⚠ THE MEMBER'S TOGGLES ARE WRITTEN BY AN RPC, AND `squad_members` STILL HAS NO UPDATE POLICY.** The obvious `for update using (user_id = auth.uid())` is wrong in a way that reads as correct: RLS gates ROWS, not COLUMNS, so it would also let any member rewrite their own `role` to `'owner'` and their own `squad_id` to any squad in the database — joining a private squad by updating a row they legitimately own, past every approval path 0050 and 0053 exist to enforce. `set_squad_notify` names the two columns it may write. **Both triggers swallow their own errors**, because `push_workout_saved` fires inside `save_workout` and a raise there would roll back the session the athlete just trained — this schema has already lost Finish Workout once to a permission change three functions away with every other gate green.

**The union's fourteen existing branches and the sender's CASE arms were TRANSFORMED from 0135's own text by a script, not retyped** (0088, 0092, 0106 and 0122 each rebuilt it from a partial read and silently deleted a shipped feature), and it is rebuilt with `create or replace` rather than dropped — the OUT columns are unchanged, so 42P13 does not apply and 0120's PUBLIC revoke survives untouched.

**⭐ AND THE BRANCH-COUNT GUARD TURNED OUT TO BE WEAKER THAN THE THING IT PROTECTS.** `push.test.mjs` matches source text, so **a branch commented out still read as present** — wrap branch 14 in `/* … */` and the sixteen-branch assertion stayed green, which is the most literal form of the exact fault it exists to catch. Found by mutation-testing rather than by anything failing. `fnBody` now strips SQL comments, and the fixed-width branch slices (`at + 600`, `at + 1400`) that started bleeding into the NEXT branch once the comments were gone are replaced by `branchOf`, which stops at the next `union all`. **Seven mutations now go red at the assertion that names them, with the paste bundle regenerated each time so nothing is caught by the byte-for-byte check instead.**

**The honor card, second pass.** `471afaa` shipped the WHY on 2026-08-12 and it is live on web and in the 17:49 UTC OTA — but the citation is joined from `honor_catalog`, and **`initiative` is granted by its own RPC (0014), has no metric or threshold, and will never be a row in it.** So the FIRST honor an athlete ever earns was the only one whose ceremony could not say what it was for, unless the tour happened to render its own hard-coded version. `fetchUncelebratedHonors` now falls back to `honorMeta`, exactly as `fetchHonorsHub` always has — and the three rule-notation triggers in that map (*"Total sessions ≥ 25"*) were rewritten as prose first, because a fallback that can reach the ceremony must not speak in schema.

**Also:** during an active workout, **Finish Workout and Add Exercise swapped places** (PO). Swapped by reordering the wrappers, not the buttons — `primaryWrap` carries `flex: 1.15` and the completion glow, so moving the children instead would have moved the emphasis without moving the button.

**Gates:** `tsc` 0 · **2,149 tests green** (26 in `push.test.mjs`, 3 new on the honor catalog) · lint at the documented baseline (1 error + 13 warnings, none in the touched files) · every new SQL guard mutation-tested.

**✅ `0153` APPLIED 2026-08-13.** ⏳ **The CODE is not deployed**, which is the state most likely to be misread as a failed migration: the database now carries the columns, the sixteen-branch union and both triggers, and the app that draws the switches is still only in the working tree. **Nothing will appear in Squad Settings until a build ships** — and even then, `eas deploy` is web-only, so the phone needs an `eas update`. The tree also carries a parallel session's uncommitted work and publishing bundles the WORKING TREE, so this cannot go out until that lands.

### 0. Playlist cover art is real, and two locked specs were amended to let it be (2026-08-13, CODE + 2 amendments — no migration)

**PO, on reviewing the feed redesign: *"Let's update that rule. Acknowledge is good."* and *"For the Spotify art, let's build that and put it in. And if we could do auto detect that would be great."*** Two locked documents said no to what was just shipped, and both have been amended rather than quietly contradicted.

**`Social-Architecture-Amendment-004-Acknowledgement-Count` (LOCKED).** SOC-D11's blanket "no popularity metric" clause is narrowed. **The distinction is between a number that describes a POST and a number that scores a PERSON** — the second stays forbidden, absolutely, along with every ranking, sorting and profile use the original clause named. Counts are hidden at zero. SOC-A4-D3 records that the four acknowledgement kinds moved to a press-and-hold rather than being deleted; SOC-A4-D4 keeps `acknowledgedLine` in the tree, unmounted, as the richer form for a post-detail surface.

**`Workout-Playlist-Amendment-002-Cover-Art-And-Recall` (LOCKED).** ⚠ **Amendment 001 forbade cover art on a premise that was false.** It reasoned that art would mean "OAuth, API integration, or SDK embedding" and that "no metadata fetch exists" — but Spotify publishes an **oEmbed** endpoint returning a title and a thumbnail for a public playlist URL with **no auth, no registered app, no token and no SDK**. The boundary was always the authentication, not the artwork. Art now renders in the feed playlist row and the attach sheet.

**⚠ THE ART IS DECORATION AND MAY NEVER BECOME LOAD-BEARING.** The bronze glyph is the row's ground state, not a spinner — offline, rate-limited, CORS-blocked on the web preview, a private playlist, a reshaped response all resolve to it, silently and finally. **`null` is cached as an ANSWER**, so a feed asks once per playlist rather than on every scroll; the request carries a 4s `AbortController` timeout; the cache is bounded at 200. **Apple Music keeps the glyph permanently** — its catalog API needs a signed developer token and `itunes.apple.com/lookup` does not serve `pl.…` playlists at all. Absent, not invented.

**⚠ THE ATHLETE'S TYPED NAME ALWAYS WINS**, and that rule is tested. The art resolves asynchronously, so the naive version replaces the label the moment it lands — somebody who typed "Leg Day Bangers" would watch the app overwrite their words with a stranger's. The fetched title fills only the gap where there was no name, in place of the generic "Spotify Playlist". `readPlaylistArt` also treats the response as **untrusted third-party JSON**: it is the only thing between a stranger's string and an `<Image src>`, so **https-only** is enforced and `javascript:`, `data:`, `http:`, `file:` and protocol-relative URLs are all refused by test.

**⚠ AUTO-DETECT IS NOT REACHABLE, and PL-A2-D6 locks that as a finding so it is not re-investigated.** iOS `MPNowPlayingInfoCenter` reports your OWN app's audio and Forge plays none; `MPMusicPlayerController` is Apple-Music-only and entitlement-gated; Android needs `MediaSessionManager` behind the **notification-listener** permission — whose usual purpose is reading every notification on the device, which is not a privacy posture to adopt for a music tag. The web preview has nothing. The one real path is Spotify's `recently-played`, which is **full OAuth** and a project of its own; it is recorded as an open item, not built.

**What shipped instead: the app remembers.** `fetchRecentPlaylists` offers the playlists this athlete has attached before, most recent first, with art, one tap to attach and no confirm step. It removes nearly the same friction and **can never be wrong about what somebody was listening to, because it only repeats what they told it before** — a detection that guesses wrong writes a false fact onto a permanent record. Deduped by URL, not by name. Absent entirely on a first-ever attach.

**Gates:** `tsc` 0 · **2,165 tests green, 0 failing** (10 new, all on what the parser must REFUSE and on the name-precedence rule) · lint at the documented baseline.

### 0. Workout Completion is two stages and three sheets, not four screens (2026-08-13, CODE — no migration)

**Design handoff `handoff/workout-complete-flow-change.md` (project `b029488a`), implemented.** The post-workout flow was Seal → Record → Reflect → Share, four early returns deep, with sharing reachable only from a chip in the corner of the ceremony. It is now **two stages on one screen** — `seal` and `capture` — plus a `record` detour, with the note, the playlist and the share destinations as bottom sheets. No route was added or removed; the whole flow is still `/workout-complete`.

**The load-bearing change is that the seal no longer leaves.** Holding used to `goHome()`, and that single fact is why every optional addition had to be offered *before* the ceremony: there was nowhere after it. The hold now sets `stage = 'capture'` 850ms later (instantly under reduced motion), and the additions live on the far side of closure where the copy can say what is actually true — *The workout is sealed. What surrounds it is still yours to add.* `overlay-branch.test.mjs` now fails if that transition ever regresses to a navigation, because if it does the capture stage becomes unreachable and the note, the photo, the playlist and Share all silently go with it, with nothing throwing anywhere.

**⚠ A REVIEWED SESSION NOW OPENS ON `capture`, NOT ON THE SEAL FACE.** Review already could not replay the ritual, so it had been reduced to a medallion with a "Done" button under it — a ceremony with its ceremony removed, whose only share affordance was the corner chip this change deletes. `capture` states exactly the condition of a session you have come back to, and it carries every write that stays meaningful later. Nothing is lost: the mark is here at 72px, the numbers are on the meta line, and the PR badges, the deltas and the record breakdown are one tap away.

**Deleted:** the Reflect step (now the note sheet, minus "Skip for today" — nothing is being skipped when the workout was sealed two screens earlier), the Share step, and the corner `ShareChip`. Share is now the only filled button on the capture stage.

**Deviations from the handoff, all deliberate and commented in place:**
- **The playlist row opens the existing attach sheet, not a list of playlists.** Workout-Playlist-Amendment-001 §7/§8A.1 rules out the integration a list would need ("no OAuth, no SDK, no metadata fetch"), so there is nothing to list. What the handoff actually insists on — *a manual picker, no auto-detection* — is what that sheet already is, and §8.5 requires both attach points to open the same one.
- **The photo row routes to `/add-photo` rather than a bare picker.** That screen is where the 75-photo / 5-video cap is enforced (M-7's pre-action check) and where a photo gets its date, label and caption. `N photos attached` is measured as a **delta** against today's chapter photos on focus, because `chapter_photos` hangs off a chapter and a date — there is no workout column, and inventing one was not this change's job.
- **Three share tiles, not two.** `Friends & Squad` is a third audience the database models directly (`(audience = 'FRIENDS') = (squad_id is null)` is an equivalence), so dropping it to match a two-tile sketch would have deleted a shipped capability.
- **`Save workout card` and `Instagram Stories` are NOT built.** Both need the card preview rendered to an image and nothing in this project can do that — there is no view-shot dependency, and a Stories deep link with no image opens an empty story. Shipping either as a row that quietly does nothing is worse than not shipping it. `Messages` (with an honest fallback to the OS sheet) and `More…` are live.

**`ShareSessionSheet` was restyled with the flow** — it is now a `BottomSheet` with an optional caller-supplied card preview above `WITHIN FORGE` / `OUTSIDE FORGE`. Activity Detail opens the same component and passes no preview, which renders no preview block rather than a placeholder.

**Gates:** `tsc` 0 · **2,149 tests green, 0 failing** (`overlay-branch.test.mjs` rewritten for the new stage set: the redundant duplicate assertion dropped, two structural guards added — the seal stage offers no sheets, and completing the seal lands on `capture` rather than navigating away) · lint at the documented baseline (1 error + 13 warnings, none in the touched files).

### 0. The feeds are a ledger, not a stack of cards (2026-08-13, CODE — no migration)

**Design handoff for `Forge Friends Feed v2.dc.html` / `Squad Detail.dc.html`, implemented.** Both feeds drew every post as a bordered, shadowed card with a **second** bronze-tinted container nested inside it for the workout numbers — two containers deep on a screen whose whole visual language is already "forged surface". Bronze was on everything, so it meant nothing, and roughly 40% of a post's height was chrome. Posts are now hairline-separated rows with an 18px gutter, no borders and no radius, and bronze appears in exactly three places: the type icon, the type label, and the stat labels.

**One renderer, two screens.** `components/forge/compositions/LedgerPost.tsx` is the whole layout; `friends.tsx` and `squad/[id].tsx` keep their own queries, their own tables and their own row shapes, and each maps into its props. **The media rules are enforced inside the component, not asked for** — a post with media renders no type marker, no title and no stat row whatever the caller passes, so neither feed can regress that independently. `RecapStrip` was deleted: it existed because two feeds rendered the same four numbers, and `workoutStats` is that one place now.

**⚠ THE SQUAD BUG THIS FIXES WAS VISIBLE FROM ACROSS THE ROOM.** A `progress` post rendered as the sentence *"Marcus Vale posted progress photos."* with **no photo underneath it** whenever the post carried loose media rather than a composed card. That sentence is attribution, not the post — it now sits above a real 4:5 full-bleed image with the member's own words below it, and the detail slot above the media is left empty.

**⚠ A SECOND REAL BUG, FOUND WHILE WIRING IT: volume was never converted in either feed.** `RecapStrip` printed `fmtVolume(summary.volume)` — canonical pounds, unlabelled — so a metric athlete read a friend's session in numbers that were neither their unit nor marked as anyone else's. `workoutStats` converts through `displayWeight` and labels the unit.

**Content, not just layout.** A workout post needs a title. `WorkoutSummary` (a JSONB snapshot, so no migration and no backfill) now carries `name` and `context`, and `fetchCompletion` returns `programName`. **"Freestyle Workout" is dropped to null** — it is the app's word for a session nobody named, and a feed of identical headings is worse than none. **`Week N · Day N` is NOT built**: nothing records which slot a saved workout satisfied (`program_sessions` is keyed by week/day and holds no workout id), so the context line names the program alone rather than guessing a week onto somebody's record.

**Deviations, all commented in place:**
- **Playlist artwork and track counts are not fetched** — Workout-Playlist-Amendment-001 §2/§7 rules out playlist metadata entirely ("no OAuth, no SDK, no metadata fetch, no cover art"). The row is real, full width and tappable, with the bronze glyph as permanent artwork and the service as the meta line. Never a partner brand colour.
- **The four acknowledgements moved to a press-and-hold.** The new action row is a single Acknowledge control whose only state change is its colour, so a tap can no longer open a chooser. Deleting Respect · Honor · Support · Strength with it would have stranded three of the four values in `post_reactions`.
- **`GOAL` posts are not built.** Neither feed has a goal post type — nothing writes one — so the marker exists in the renderer and no data path reaches it. The `Forged in Forge Legacy` badge §4 asks to delete is not on a feed post; it is a line in `domain/share/content.ts`'s exported text and was left alone.
- **`backdrop-filter` is not applied** to the photo counter or the play disc; React Native has no such style. Both keep the specified translucent fill and hairline.

**⚠ OPEN DECISION: the count beside Acknowledge contradicts SOC-D11**, which says an acknowledgement names people and "no count is ever rendered as a score" — the reason `acknowledgedLine` exists. The handoff is explicit about the count and PD-7 makes the design govern, so it is built as drawn; `acknowledgedLine` is left in place, unmounted, carrying a comment saying why. **The Social architecture needs amending to match.**

**Gates:** `tsc` 0 · **2,149 tests green, 0 failing** · lint at the documented baseline.
...
**⚠ STILL NOT APPLIED AS OF 2026-08-13, and the PO felt it.** *"I've clicked preview on it and also I've clicked skip and it keeps popping up."* Because `created_at` never arrived, the window above has never fired once and the card has been living out the full week ever since. Paste-ready bundle: `supabase/apply/pending-0152-weekly-review.sql`. See the 2026-08-13 entry above for the two other causes and the device-local retirement that no longer depends on this migration.
...
**⚠ WHY EVERY GATE PASSED.** `verify-client-rpcs-still-reachable.sql` enumerates the RPCs `src/` names directly — 78 of 79 green. `evaluate_honors` has **zero client call sites**; it is only ever reached from inside another function, which is exactly why 0147 believed it was internal and exactly why the verifier could not see it. **A grant audit must walk in-database callers and their security modes, not just the client's call list.** `0147` is amended: the entry is removed, the wrong reasoning is kept in place and marked wrong (the mistake is more useful than a clean file), and its §5 counter no longer counts `evaluate_honors`.

**The fix is a grant, not a definer flip.** Making `save_workout` SECURITY DEFINER would also clear the error and would take the whole save path out from under RLS to repair a grant. `0150` grants `evaluate_honors(text)` to `authenticated` only — `anon` stays revoked and the migration asserts both directions plus `honor_metrics` staying closed. Not a widening: the function derives its subject from `auth.uid()` alone (0099:551), raises on a null uid, is idempotent on re-run via its two partial unique indexes, and carried this grant from 0012 until 0147.

### 0. Full launch audit — and the database had been open to strangers for three years (2026-08-12, AUDIT + migrations `0146` · `0147` — ✅ **BOTH APPLIED** + CODE)

**Nine parallel auditors, one per domain, every user action traced screen handler → data module → table/RPC → the migration that defines it.** Full ranked list: **`Docs/Launch-Audit-2026-08-12.md`** — 33 P0, ~40 P1. Four findings were reproduced by executing the real modules under `node --test`, not by reading. Baseline going in: `tsc` 0, 2,068 tests green, and **every blocker below was present anyway** — they all live in the gap between a tested helper and the screen that should call it. `customToPickerItem` and `mergeForSearch` have unit tests and no screen call site at all.

**⚠ THE SECURITY FINDING, AND IT IS THE BIG ONE.** `honor_metrics(uuid)` was callable by **anon** for any athlete id — its guard is `auth.uid() is not null and p_uid <> auth.uid()`, which short-circuits to false when `auth.uid()` is null. Closing it exposed the real defect: **all 33 `revoke execute … from public` statements in this schema had never done anything.** There are TWO grants — PostgreSQL's own `PUBLIC` default and Supabase's direct `anon` grant from `alter default privileges` — and removing either alone leaves the other. `0147` removes both and turns off the platform default so the next migration cannot re-grant it. **Before the fix, ~95 SECURITY DEFINER functions were reachable with no account**: any athlete's notification stream by id, profiles, trophy cases, live training status, friend lists, squad record books, **invite codes**, plus `push_drain` / `app_events_prune` / `squad_checkin_prune` — drain everyone's notifications or delete check-in videos. Verified closed: `anon_project_fns = 0`, and all **78 of 79** client RPCs still reachable by `authenticated` (`coach_ai_balance` is 0144, correctly unapplied).

**⚠ `notification_events_for` had been re-revoked FIVE times — 0120, 0121, 0122, 0126, 0135 — each with a comment warning that omitting the line would re-open the escalation. The line was always there. It never worked.** This is the fourth recorded way a migration lies about having worked, and the only reason it surfaced is that `0146` **asserted its own outcome instead of trusting its own statement**. Both 0146 and 0147 failed their own assertions on first run (three times total) and rolled back cleanly. **Every migration from here asserts its result, and returns ROWS — the Supabase SQL editor does not display `raise notice`, so every self-check this repo has ever written has been invisible.**

**`0146` also closed two storage holes, one of which nobody reported.** `transformation-media` UPDATE/DELETE were scoped by `bucket_id` alone — any authenticated athlete could delete or overwrite **any other athlete's six progress poses and posing video**. `0075_media_owner_scope.sql` diagnosed this exact shape on `squad-media`, fixed it, and wrote down why; 0044 predates it and was never revisited. **Sweeping every `storage.objects` policy then turned up a third bucket** — `squad-photos`, whose policies are literally *named* `owner_update` / `owner_delete` and have never checked an owner. That one is scoped by **`squads.owner_id` via the path, not by `storage.objects.owner`**: the upload writes a FIXED path with `upsert: true`, so owner-scoping it would mean a squad that changed hands could never change its photo again.

**FIXED IN CODE: continuing a workout could silently discard everything just logged.** `buildAppendExercises` mapped over the FILTERED rows while indexing `session.exercises[i]`, the UNFILTERED list, so one un-logged cardio block re-paired every later row. Reproduced: three logged sets of Barbell Row → `buildAppendExercises` returned `[]`, `continue_workout` reported `sets_added: 0`, raised nothing, and the athlete was shown the completion screen. `buildSubstitutions` already carried a comment warning about exactly this trap. **A comment is a thing to remember; `recordedExercises()` is now the one list all three writers walk, so they cannot drift.** Six regression tests in `append-alignment.test.mjs`, **mutation-verified** — two fail on the old code.

**THE PATTERN UNDER ~169 FINDINGS IS ONE MISTAKE REPEATED: a correct fix landed at one call site while its siblings were missed** — and in three cases the migration that made the fix wrote down the reasoning that would have found the others. `0075` → transformation-media · `0108` trained-today → Discover + Preview still count check-ins only · `0052` member cap → `join_squad_by_code` has none · `0135`'s revoke → `honor_metrics` · `preferences.tsx`'s rollback → 13 other toggles · `useUnits` → **34 of 48 weight/distance surfaces do not convert**. Remediation must grep every sibling call site, not patch the reported instance.

**Still open and NEEDING A PO RULING** (§4 of the audit doc): (1) making photos genuinely private needs signed URLs across every media surface — the client builds every image link with `getPublicUrl()`, so a bucket flip 404s every photo in the app; (2) hiding `squads.invite_code` and `profiles.training_since` needs a column-level grant, which must enumerate every remaining column and silently breaks reads when a future column is added; (3) **the distance-units doctrine conflict** — `units.ts:8-10` says distance must never convert, `0139:10-14` says `units` is the single switch, and the code follows both in different files; (4) **account deletion — App Store Review 5.1.1(v) requires it in-app and `content.ts:44` already promises it. Submission blocker.**

**Gates after:** `tsc` 0 · **2,093 tests green** · new diagnostics at `supabase/apply/preflight-launch-audit.sql`, `audit-anon-executable-functions.sql`, `verify-client-rpcs-still-reachable.sql` (the old `preflight-what-is-applied.sql` labelled `app_admins` as 0137 when **0129** creates it, so the row went green whenever 0129 ran and 0137 did not, and it never checked the push cron jobs at all).
