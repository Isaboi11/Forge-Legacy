...
`tsc` 0 · **2,461 tests green** (11 new in `cardio-manual-log.test.mjs`) · lint at baseline (1 error + 13 warnings, both pre-existing) · **both fixes mutation-tested** — reverting each one individually fails exactly the tests written for it. **Not yet deployed:** the working tree carries another session's uncommitted P-8/billing work, and publishing bundles the working tree.
