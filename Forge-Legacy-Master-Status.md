...
| **35** | **Holt's voice — the coach you hired** *(new 2026-09-21 · rewrite BUILT same day)* | `Docs/Amendments/Holt-Voice-Amendment-001-The-Coach-You-Hired.md` 🔒 **LOCKED, tone approved by the PO.** Holt is encouraging but not cheesy, pumped for real wins, and warm about keeping going, in both the scripted and the AI Holt. **Built:** 771 scripted lines (up from ~320). Each line deals every version before repeating, that memory is saved on the device (`lib/voice-memory.ts`), greetings follow the time and day, and the workout coaching line (`progressionFor`, previously one fixed sentence per verdict) now varies week to week, is pinned within a session and follows the intensity dial. tsc 0 · 3,514/3,514. ⛔ **Not committed, not deployed, not seen.** | Commit + OTA when the PO wants it on the phone |
