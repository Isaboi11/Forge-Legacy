...
**2,419 tests green, tsc clean, lint at baseline (1 error + 13 warnings, all pre-existing).** No native config changed, so this reaches the phone by `eas update` rather than a new build — **`fingerprint:compare` against the live build before publishing**, per the trap in § OTA.
