...
### 0. The morning briefing — the first notification in the app that nobody causes (2026-08-14, CODE + migration `0159` — ⚠ **AUTHORED, NOT APPLIED** · ⚠ NOT DEPLOYED)
...
**⚠ NOT APPLIED AND NOT DEPLOYED.** 0159 must be pasted into the SQL editor (bundle carries a verify checklist). Applying it sends **nothing** to anybody — `training_briefing` defaults OFF and `briefing_schedule` is created empty. Client side is JS-only, so it ships by **OTA**; `expo-notifications` is already in the build. **Do not publish from this tree until the parallel run-tracking work is committed or stashed** — publishing bundles the working tree.
