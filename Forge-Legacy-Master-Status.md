...
| **Backend / Data** | **BUILT (Supabase) — 158 migration files, 0001–0158. ✅ APPLIED AND VERIFIED THROUGH 0154** (preflight 2026-08-13, **24/24 green**; `0144` correctly absent by decision). ⏳ **`0155`–`0158` AUTHORED, NOT APPLIED, NOT COVERED BY ANY PREFLIGHT** — `0155_program_state_finished` · `0156_short_program_credit` · `0157_week_templates` · `0158_short_program_cap`, all from `23ab50a`. ⚠ **THIS ROW WENT STALE WITHIN THE HOUR IT WAS CORRECTED** — it was rewritten at 0154 and a concurrent session committed four more before the day ended. That is not carelessness, it is the structural fact this board keeps losing to: **the ledger is edited by hand and the schema is not, so any number written here is true only at the instant it is typed. Run the preflight; do not read this cell.** The `0155`–`0158` checks still need adding to `supabase/apply/preflight-0146-0153.sql` | ⚠ **This row read "125 files, applied through 0125" while eighteen more had shipped** — the drift is recorded because it is the recurring failure of this board, not a one-off. 0137 signup alerts · 0138 substitution + avoidance capture · 0139 every athlete to imperial · 0140 athlete weekly reviews · 0141/0142 squad check-in video prune + orphan ledger · 0143 coach intensity signals. **Verified by the PO from the SQL editor**, not assumed: `exercise_avoidance`, `athlete_weekly_reviews` and `coach_intensity_signal` all return 0 rows without error, and `profiles` off imperial = **0**. ⚠ **0141 shipped able to run only ONCE** — `create or replace` cannot change a return type and 0142 changes it, so a re-run died on `42P13`; fixed to DROP first, because with no CLI and no history table, re-running from the top is the only recovery this project has. RLS on every table; every `SECURITY DEFINER` pins `search_path` |
