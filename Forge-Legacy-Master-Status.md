...
### 0. ⭐ Holt makes a call, instead of offering twelve buttons — the mid-workout sheet gets a hierarchy (2026-08-22, Coach Holt / W-9 — no migration, ✅ WEB DEPLOYED + ✅ OTA DELIVERABLE ON BUILD 6)
...
✅ **DEPLOYED AND VERIFIED ON BOTH SURFACES.** Web `entry-83c2c18dd83efe897a4e07fb8dd766ea.js` — 200
twice with a matching hash, and the **live bundle searched for seven strings only this code contains**
(`Why this weight?`, `IF I HAVE IT WRONG`, `REBUILDING`, `GOING UP`, `Balance today`, `See all
alternatives`, `ADJUST TODAY`) — all present. iOS OTA `01a02cd3-0943-784c-85b7-65d63e7e262b` on runtime
`411fd2b6…`; `fingerprint:compare --build-id 078d2838…` an **exact match with build 6 before
publishing**, manifest endpoint then queried as an iOS client and **returned the new id**. Commit
`207e4c2`.

⏳ **Not device-confirmed.** This is a visual change and **no test in this repo can see it** — the
treatments, the 56pt rows and the collapse behaviour are wired correctly, which is a different claim
from "it feels calmer in a gym".
...
The 45 entries before this point moved there on 2026-08-18, 2026-08-19, 2026-08-20 and 2026-08-22, **verbatim**. Nothing was deleted or
summarised.
