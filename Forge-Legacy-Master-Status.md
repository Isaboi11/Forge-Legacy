...
### 0. ⭐ See what a friend has logged and has planned while they train — and the share confirmation stays until Done (2026-08-27, Live Workout / Privacy / Share sheet — **MIGRATION `0181` AUTHORED, NOT APPLIED**, ✅ **WEB DEPLOYED** `index-fabbdade2704478e0bbfaf5e90dffa0d` · ✅ **OTA PUBLISHED TO BUILD 7** iOS `01a0469c-b9f0-7a86-ab02-520e326ddbb7`, runtime `4d728d16…` — commits `462a287` `fbeceb0`)
...
⛔ **`0181` IS NOT APPLIED.** `supabase/apply/pending-0181.sql` is the paste bundle — the migration embedded verbatim, §2 raises if anything is missing, **§3 predicted: `live_rows 0 · athletes_opted_in 0 · reader_is_definer true · policies 1`.** Until it lands every read says "isn't sharing" and every publish is a no-op; nothing breaks. **Applying ≠ working**: after the paste, someone flips Live Workout Detail on, trains, and a friend opens the row — that is the proof.
