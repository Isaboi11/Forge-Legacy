...
**Last Updated:** 2026-08-20 (**⭐ THE AGE-RATING QUESTIONNAIRE HAD CHANGED, AND ANSWERING IT FROM MEMORY WOULD HAVE BEEN WRONG.** Apple replaced it in 2025: the tiers are now **4+ / 9+ / 13+ / 16+ / 18+** — *12+ and 17+ no longer exist* — and it is split into **In-App Controls · Capabilities · Content Descriptors**. Answered question by question in `Docs/App-Store-Listing-Copy.md` **§6b** with Apple's own definitions quoted beside each answer: **`13+`, `16+` in Australia**, driven twice over by *Social Media = Present* and *Contests = Frequent*, so it does not hang off one debatable box. ⚠ **From September 2026 the Social Media questions are REQUIRED to submit at all** and we submit inside that window. Two answers are judgement calls and both are argued in writing — *Medical or Treatment Information = None* is **conditional on `limitations.ts` never drifting into injury advice**, since the alternative is 16+. In the same pass **`0173`** seeds the moderation filter list `0171` left owed — 37 patterns, and **eighteen documented exclusions that are the harder half**: the match is a substring with no word boundary, so **`rapist` is inside `therapist`** and **`pedo` is inside `pedometer`**, both handles a real athlete on a *fitness* app would pick. Verified in both directions — 8 known-bad rejected, 30 known-good untouched — because a blocklist that only proves it catches slurs has proved half of nothing, and a wrongly blocked signup is **silent**. ⏳ **`0173` is authored, NOT applied.** ⚠ **And `0172` is in the tree with a paste bundle and recorded nowhere** — it re-applies the per-column grants `0169` skipped, and until it runs, deploying `0169`'s client half raises **42501 on every read of `profiles`**.)
...
| **Last Updated** | 2026-08-20 (**The two listing blockers close.** The **age rating is answered — `13+`, `16+` in Australia** — against a questionnaire Apple **replaced in 2025**: 12+ and 17+ are gone, and it is now In-App Controls · Capabilities · Content Descriptors. Sheet in `Docs/App-Store-Listing-Copy.md` **§6b**. ⚠ From **September 2026** the Social Media questions are required to submit. And **`0173`** seeds the filter list `0171` left owed — 37 patterns plus **eighteen documented exclusions**, because the match is a substring: `rapist` is inside `therapist`, `pedo` is inside `pedometer`. ⏳ **`0173` authored, NOT applied · `0172` recorded nowhere and presumed unapplied — it is a deploy blocker for `0169`'s client half.**) |
...
The 40 entries before this point moved there on 2026-08-18, 2026-08-19 and 2026-08-20, **verbatim**. Nothing was deleted or
summarised.
