-- ═══════════════════════════════════════════════════════════════════════════════════════════════
-- Forge Legacy — PASTE-READY BUNDLE: migration 0127 (a hold reads back as a hold)
--
...
--
-- ⚠ NEEDED BY THE SAME RELEASE THAT STOPS LOGGING A 60s PLANK AS 10 REPS. A timed strength set now
--   writes `workout_sets.duration_sec`. Without this migration `save_workout_as_template` reads that
--   duration as evidence of CARDIO and turns every hold into a run.
--
-- ⚠ REBUILT FROM THE NEWEST BODIES: `save_workout_as_template` from 0106 (not 0097),
--   `shared_workout_detail` from 0117. `save_workout` is deliberately NOT touched.
--
-- VERIFY AFTER RUNNING:
--   -- log a session with a timed exercise, save it as a template, and confirm the hold is a strength
--   -- row carrying `targetDurationSec` rather than a cardio block.
-- ═══════════════════════════════════════════════════════════════════════════════════════════════
