-- ═══════════════════════════════════════════════════════════════════════════════════════════════
-- PENDING — 0202: leaving a workout stops re-announcing it, and every notification starts ON
--
-- PASTE THIS WHOLE FILE into the Supabase SQL editor and run it once.
-- Safe to run twice: every statement is guarded, a restatement, or narrowed to match nothing again.
-- ═══════════════════════════════════════════════════════════════════════════════════════════════
--
-- ══ WHAT THIS IS FOR ══
--
-- PO, 2026-09-20, two things:
--
...
--
-- ⚠ THE FIRST IS NOT 0187 FAILING. 0187 made `training_since` write-once per session and that still
--   holds. It closed the case it was written for (the app killed mid-session, the client re-announcing
--   on mount). It cannot close this one, because leaving does not re-announce over a live stamp — it
--   CLEARS it, on purpose, so an athlete who backs out is not lit on Live Now for four hours. Resuming
--   then writes a fresh `now()`, which is a genuinely different outbox key, which is a second push.
--   `profiles.training_announced_at` carries the session's identity across the gap. Presence is
--   unchanged: leaving still takes you off Live Now immediately.
--
-- ⚠ THE SECOND IS THE HALF 0189 DEFERRED IN WRITING. Squad announcements have always had a sender —
--   every post type is a `squad_posts` row and branch 10 fans it out to every member — and the default
--   for `squad_feed` has been OFF since 0122, so all of them were discarded before reaching a device.
--   This is the same deliberate, temporary override of `P-5 §3.1` that 0189 applied to training alerts.
--
-- ⛔ NOTHING IS SENT RETROACTIVELY. An announcement posted before this is pasted stays unannounced on
--    the device. It DOES appear in /inbox the moment this lands (branch 10 is read-time, 14-day window),
--    with no new build and no push.
--
-- ⚠ TWO ORDERING NOTES:
--
--   · `0195_coaching_notifications` and `0196_trainer_messages` on `feat/forge-coach` ALSO restate
--     `push_pref_key` and `push_pref_default`, written and not applied. Whichever is pasted LAST wins,
--     WHOLE-BODY. Paste those after this one and these defaults revert silently — re-paste §3 of this
--     file afterwards if you do.
--   · The app is safe to deploy BEFORE this is pasted. The client falls back to the two-argument call on
--     PGRST202, so presence keeps working; only the duplicate announcements persist until this runs.
--
-- ═══════════════════════════════════════════════════════════════════════════════════════════════
...
-- ═══════════════════════════════════════════════════════════════════════════════════════════════
-- READ-ONLY — what the paste changed. One row, every answer.
--
-- ⚠ The editor shows only the LAST statement's result, which is why this is one row and not five
--    selects. The self-check above RAISES on anything wrong, so reaching this output already means
--    the migration held; this is here to see the SIZE of what moved.
-- ═══════════════════════════════════════════════════════════════════════════════════════════════

select
  -- Expect true, true, true, false. The fourth is deliberate — see the migration header.
  public.push_pref_default('squad_feed')                                    as squad_posts_push_expect_true,
  public.push_pref_default('squad_reactions')                               as reactions_push_expect_true,
  public.push_pref_default('squad_activity')                                as membership_push_expect_true,
  public.push_pref_default('training_briefing')                             as briefing_push_expect_false,

  -- Athletes who had explicitly stored `false` and were turned back on by §4.
  (select count(*) from public.profiles p
    where (p.notif_prefs ->> 'squad_feed') = 'true')                        as profiles_with_squad_feed_true,

  -- The new column, and the two it sits beside. All three must be invisible to a signed-in athlete.
  has_column_privilege('authenticated', 'public.profiles', 'training_announced_at', 'select')
                                                                            as authed_sees_announced_expect_false,
  has_column_privilege('authenticated', 'public.profiles', 'training_since', 'select')
                                                                            as authed_sees_presence_expect_false,

  -- Exactly one signature, taking three arguments.
  (select count(*) from pg_proc p join pg_namespace n on n.oid = p.pronamespace
    where n.nspname = 'public' and p.proname = 'set_training_status')       as set_training_status_overloads_expect_1,

  -- Anybody mid-session right now keeps their stamp; nobody should have an announcement without one
  -- unless they are between a leave and a resume, which is the state this migration exists to hold.
  (select count(*) from public.profiles p
    where p.training_announced_at > now() - interval '4 hours')             as sessions_announced_last_4h,
  (select count(*) from public.profiles p
    where p.training_since > now() - interval '4 hours')                    as athletes_training_now;
