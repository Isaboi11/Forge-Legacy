-- ═══════════════════════════════════════════════════════════════════════════════════════════════
-- Forge Legacy — PASTE-READY BUNDLE: migration 0126 (squad weekly review notification)
--
...
--
-- ⚠ RUN 0122 FIRST. This rebuilds `notification_events_for` from 0122's ELEVEN-branch body plus one.
--   Applied out of order it would silently delete `squad_post` and `squad_checkin` — the exact fault
--   0088, 0092 and 0106 each shipped.
--
-- ⚠ TWELVE BRANCHES after this runs. Count them in the union before editing it again.
--
-- WHAT IT FIXES: the weekly recap is an AUTHORLESS `squad_posts` row (0057), and both
-- `author_id <> p_user` (the union) and `user_id <> author_id` (the push trigger) evaluate to NULL
-- against it — so the review has never notified anybody, in the inbox or by push.
--
-- VERIFY AFTER RUNNING:
--   select kind, count(*) from public.notification_events_for(auth.uid()) group by kind;
--   -- open a squad whose weekly recap has generated; `squad_recap` should appear.
-- ═══════════════════════════════════════════════════════════════════════════════════════════════
