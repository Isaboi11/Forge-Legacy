-- ══════════════════════════════════════════════════════════════════════════════════════════════════
-- PENDING — 0172 + 0173, IN THAT ORDER. One paste.
--
-- PASTE THIS WHOLE FILE into the Supabase SQL editor and press Run ONCE. Both halves are idempotent
-- and safe to run twice.
--
-- ⚠ IT IS ALL-OR-NOTHING, AND THAT MATTERS WHEN READING A FAILURE. The SQL editor sends the whole
--   file as one simple-query string, which PostgreSQL runs in a SINGLE IMPLICIT TRANSACTION. So if
--   0173's self-check raises, **0172's grants roll back with it** and NEITHER migration is applied.
--   Do not read "0173 failed" as "0172 landed". Re-run the whole file after fixing, never half of it.
--
-- ⚠ THE ORDER IS NOT COSMETIC. 0172 is the deploy blocker; 0173 is data. 0172 goes first so that if
--   it stops the run (see below), nothing else has half-happened.
--
-- ⚠ 0172 STOPS THE RUN DELIBERATELY if `reapply_hidden_column_grants()` is absent — that means 0160
--   was never applied, and it raises rather than guessing. If you see that message, stop and say so;
--   do not delete the check.
--
-- WHAT YOU SHOULD SEE, in the Messages pane, in this order:
--   0172: <table> — N columns granted, M kept hidden   (one line per table)
--   0172 OK: profiles.experience and profiles.training_goals are selectable, and squads.invite_code is still hidden.
--   0173 OK: 37 patterns landed, 8 known-bad rejected, 20 known-good survive.
--
-- AND ONE RESULT ROW, which is 0173's §3. Predicted before running:
--   total 46 · both 13 · handle 33 · name 0 · added_here 37 · existing_profiles_now_failing 0
--
--   ⚠ `existing_profiles_now_failing` is the one number worth pausing on. The trigger is NOT
--     retroactive, so a non-zero count is not an outage — it is an athlete who keeps their handle until
--     the day they try to edit their profile, and then cannot save. Expected 0 on a cohort of twenty.
--     If it is not 0, paste the number back before anyone changes anything.
--
-- AFTER THIS RUNS: 0173 needs no deploy — its trigger is already live and the client already surfaces
--   the P0001 message. 0172 needs no deploy either, but it UNBLOCKS one: `0169`'s client half
--   (`coach-profile-live.ts`) can only ship once these grants exist.
--
-- Source files, carried in verbatim and unedited:
--   supabase/apply/pending-0172.sql
--   supabase/apply/pending-0173.sql
-- ══════════════════════════════════════════════════════════════════════════════════════════════════
