-- ═══════════════════════════════════════════════════════════════════════════════════════════════
-- Forge Legacy — PASTE-READY BUNDLE: migration 0163 (competitions — the standings read and the invite)
--
...
--
-- WHAT IT FIXES — two PO reports, both server-side, neither a regression:
--
--   1. "It said it could not load — column t2 is ambiguous." That is `tz`, and `challenge_detail()`
--      has raised 42702 on EVERY call since 0099 added `profiles.tz` and collided with the bare
--      plpgsql local of the same name. C-3 has never loaded, for anyone. The hub was unaffected
--      because it has no such local, which is why this presented as a screen bug.
--
--   2. "I added someone and it did not send them an invite." It could not: Create Challenge defaults
--      to "Starts Today" = midnight this morning, `advance_challenges()` runs the moment the creator
--      lands back on the hub, and the invite event and the Join row were both gated on ENROLLMENT.
--      The derived notification stopped existing seconds after the competition was made.
--
-- ⚠ RUN AFTER 0162. It rebuilds `notification_events_for` from 0153's body with ONE predicate changed
-- — all sixteen branches carried forward. Applying it to a database on an older copy would roll that
-- function back for every other notification, the failure 0088, 0092 and 0106 each shipped.
--
-- SAFE TO RUN TWICE: every statement is `create or replace`, and the assertion block at the end only
-- reads `pg_get_functiondef`.
--
-- VERIFY AFTER RUNNING — the migration self-checks, so a clean run already proves the edits landed:
--   select public.challenge_hub() -> 'open';   -- a competition you were invited to and have not joined
--   -- and then, in the app, open a competition: it loads instead of erroring.
-- ═══════════════════════════════════════════════════════════════════════════════════════════════
