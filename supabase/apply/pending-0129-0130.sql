-- ═══════════════════════════════════════════════════════════════════════════════════════════════
-- Forge Legacy — CREATOR DASHBOARD (migrations 0129 + 0130)
--
--   1. Supabase Dashboard  →  SQL Editor  →  New query
--   2. Paste this ENTIRE file
--   3. Run
--
-- That's it. Nothing in here needs editing, and it is safe to run more than once.
--
-- The last block grants YOU admin by looking up your email, so there is no user id to copy. If you
-- ever want a second operator, add their email to that list and run the file again.
--
-- After it runs, open the app and go to Settings — a "Creator Dashboard" row appears at the bottom.
-- It appears only for accounts in `app_admins`; every other athlete's Settings is unchanged.
--
-- WHAT THIS CREATES
--   · app_admins        — the operator allowlist. RLS on with ZERO policies, so nobody can read it.
--   · is_app_admin()    — "am I an operator?"
--   · admin_guard()     — the refusal, called first inside every function below.
--   · seven admin_* read models — overview, growth, retention cohorts, engagement, feature adoption,
--     content popularity, social health. Population totals only; no athlete is ever named.
--
-- IF SOMETHING GOES WRONG mid-run, just paste the whole file again from the top.
-- ═══════════════════════════════════════════════════════════════════════════════════════════════
...
--
-- This runs as `postgres` in the SQL editor, which can read auth.users, so it finds your id from your
-- email and you never have to copy a uuid. Idempotent: running it again changes nothing.
--
-- ⚠ You must have signed up in the app already — this matches an EXISTING account. If the verify
--   block below reports 0 rows, the email did not match one; check it and re-run this block.
-- ═══════════════════════════════════════════════════════════════════════════════════════════════

insert into public.app_admins (user_id, note)
select u.id, 'creator — bootstrap'
  from auth.users u
 where lower(u.email) in (
   -- Add another line here to grant a second operator. One email per line, lowercase, comma-separated.
...
-- ═══════════════════════════════════════════════════════════════════════════════════════════════
-- VERIFY — expect ONE row, showing your handle.
--
-- If you get 0 rows, the email above did not match an account in auth.users. Run
--   select email from auth.users;
-- to see the exact address you signed up with, put it in the list above, and re-run STEP 2.
-- ═══════════════════════════════════════════════════════════════════════════════════════════════

select p.handle, p.name, a.granted_at
  from public.app_admins a
  join public.profiles p on p.id = a.user_id;

-- Optional smoke test. In the SQL editor `auth.uid()` is null, so admin_guard() will refuse this —
-- that refusal IS the gate working. The real proof is opening Settings in the app.
-- select jsonb_pretty(public.admin_overview(30, 'America/Denver'));
