-- ═══════════════════════════════════════════════════════════════════════════════════════════════
-- PENDING — 0171: blocking, reporting and filtering (App Store Guideline 1.2)
--
-- PASTE THIS WHOLE FILE into the Supabase SQL editor and run it once.
-- Safe to run twice: every statement is guarded, and the closing SELECT is read-only.
--
-- ⚠ APPLY 0170 FIRST if you have not. This file does not depend on it, but the ledger should stay in order.
--
-- ⛔ THIS IS A SUBMISSION BLOCKER, NOT A FEATURE. Guideline 1.2 requires an app with user-generated
--    content to offer filtering, reporting WITH timely response, blocking, and published contact details.
--    Before this migration Forge had the last of those and ONE report control — a toast in squad settings
--    reading "Reporting a squad is coming soon".
--
-- ⚠ NOTHING BELOW CAN BE VERIFIED FROM THE SQL EDITOR by calling it: every function keys off `auth.uid()`,
--    which is null here, so each raises 28000. The closing SELECT is catalogue reads only. Use
--    `supabase/apply/verify-0171.sql` after applying, and the app itself for behaviour.
-- ═══════════════════════════════════════════════════════════════════════════════════════════════
