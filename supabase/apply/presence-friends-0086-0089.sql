-- Forge Legacy — apply bundle: 0086 → 0089, IN ORDER.
--
-- NOT new migrations. All four are numbered in supabase/migrations/; this file exists so the whole chain
-- can be pasted in one go.
--
-- ORDER IS NOT OPTIONAL. 0087 adds `invited_ids`, which 0088's notification derivation reads; 0089 reads
-- `training_since` from 0086. Out of order fails with `42703: column does not exist`.
--
--   0086  training presence — profiles.training_since + set_training_status() + training_now()
--   0087  competitions between friends — invited_ids, the FRIENDS branch through every policy, and
--         challenge_hub()/advance_challenges() generalised off squad-only
--   0088  the invite notification — notification_events/feed/unread_count DROPPED and rebuilt (the
--         return type gains challenge_id, which create-or-replace cannot do)
--   0089  athlete_training_status() — what makes the "Everyone" audience real
--
-- SAFE TO RE-RUN: columns are `if not exists`, constraints are dropped before being re-added, policies
-- are dropped before being recreated, and every function is create-or-replace.
