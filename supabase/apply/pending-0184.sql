-- ═════════════════════════════════════════════════════════════════════════════
-- PASTE BUNDLE — 0184. Paste this WHOLE FILE into the Supabase SQL editor at once.
...
--      supabase/migrations/0184_rename_squad_post.sql
-- ═════════════════════════════════════════════════════════════════════════════

-- 0184 — an athlete can name, and rename, their own post.
--
-- ══ WHY ══
--
...
-- composition this title belongs to. So the name goes in it — the same trick 0183 used for the route
...
-- snapshot, the route consent 0183 just added. Renaming does not need that and must not buy it.
--
...
begin
  ...
  if src is null then
    raise exception '0184 DID NOT APPLY. public.rename_squad_post does not exist.';
  end if;
  ...
  if missing <> '' then
    raise exception '0184 DID NOT FULLY APPLY. Missing:%', missing;
  end if;

  raise notice '0184 OK — rename_squad_post is present, security definer, author-checked.';
end $verify$;
...
begin
  if exists (select 1 from pg_policies where schemaname = 'public' and tablename = 'squad_posts' and cmd = 'UPDATE') then
    raise exception '0184: an UPDATE policy exists on squad_posts. That was never intended — see the header.';
  end if;
  raise notice '0184 OK — squad_posts still has no UPDATE policy, as designed.';
end $noupdate$;
