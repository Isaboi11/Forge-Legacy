...
-- `planned_workouts` slot (0136) — on the Home hero, waiting to be started. The driving case is the
-- nightly drip: Squatober and everything shaped like it, where the organiser posts tomorrow's session
-- the night before rather than publishing a plan in advance.
--
-- Governed by `Squad-Architecture-Amendment-005-Posted-Workouts.md` (🔒 LOCKED 2026-09-03).
--
-- ══ ⚠ WHAT THIS DELIBERATELY DOES NOT TOUCH — READ BEFORE "IMPROVING" IT ══
--
--   * **`squad_feed()` and the post-detail read are NOT rebuilt.** Both are `returns table (...)`, which
--     `create or replace` cannot widen. The workout snapshot rides in `squad_posts.layout` (untyped
--     jsonb, 0045) which every read path already returns and which already carries a `kind`
--     discriminator. No column, no backfill, no read path changed. 0186's header explains this wall at
--     length; 0043 hit it first.
--
--   * **`notification_events_for()` is NOT rebuilt.** Its branch 10 already fans out every authored
--     squad post to every member, so a 'workout' post notifies the moment §1's constraint admits it —
--     under the existing Squad Feed Activity toggle, no new preference (SQ-A5-D8). That function has
--     silently lost shipped features to a from-memory rebuild FOUR times; not touching it is the whole
--     point.
--
--   * **Nothing counts takes.** No counter, no takes table, no aggregate (SQ-A5-D5.2). The number is
--     absent rather than merely unshown.
--
--   * **`take_posted_workout` is SECURITY INVOKER, not DEFINER.** Reading the post is gated by
--     `squad_posts_select` (member-only) and writing the slot by `planned_workouts_own_*` (own row), so
--     RLS enforces both halves. Making it definer would move the membership check out of the database
--     and into this function, which is the reflex that has caused real outages here.
--
-- ══ WHAT THIS FILE DOES ══
--
-- §1  the migration, verbatim from `supabase/migrations/0192_posted_workouts.sql` — 15 statements:
--       · the 'workout' post type, with the full existing list carried forward (0074/0076's scar)
--       · a BEFORE INSERT trigger: one posted workout per member per squad per UTC day (SQ-A5-D1.3)
--       · three nullable provenance columns on `planned_workouts`, all `on delete set null`
--       · `take_posted_workout(uuid)`, its revoke/grant, and four comments
-- §2  asserts the constraint, the trigger, all three columns and the function exist — RAISES if not
-- §3  reports what landed and how much of it is in use. Read-only.
--
-- ══ WHAT §3 SHOULD SAY (write this down before you run it) ══
--
--   workout_posts        = 0   — the composer that writes them is not deployed yet
--   taken_slots          = 0   — nothing can have been taken
--   planned_slots_total  = whatever athletes have built for themselves; UNCHANGED by this migration
--   type_allows_workout  = true
--   trigger_present      = true
--   function_present     = true
--
-- ⚠ A NON-ZERO `workout_posts` OR `taken_slots` MEANS SOMETHING IS ALREADY WRITING THESE, which on a
-- database whose client has not shipped the feature would mean this file is being run somewhere it was
-- not expected — stop and find out what before going further.
--
-- ═══════════════════════════════════════════════════════════════════════════════════════════════


-- ═════════════════════════════════════════════════════════════════════════════
-- §1 — THE MIGRATION (verbatim from supabase/migrations/0192_posted_workouts.sql)
-- ═════════════════════════════════════════════════════════════════════════════
...
declare
  missing text := '';
begin
  -- The type constraint must admit 'workout' — and must still admit everything it did before, which is
  -- the failure 0076 had to repair after 0074 rebuilt this list from memory.
  if not exists (
    select 1 from pg_constraint
     where conname = 'squad_posts_type_check'
       and pg_get_constraintdef(oid) like '%''workout''%'
  ) then missing := missing || ' squad_posts_type_check/workout'; end if;

  -- ⚠ And it must not have LOST one on the way. This is the 0074 regression as an assertion.
  if not exists (
    select 1 from pg_constraint
     where conname = 'squad_posts_type_check'
       and pg_get_constraintdef(oid) like '%''formcheck''%'
       and pg_get_constraintdef(oid) like '%''transformation''%'
  ...
  ) then missing := missing || ' squad_posts_type_check/LOST-AN-EXISTING-TYPE'; end if;

  if not exists (
    select 1 from pg_trigger where tgname = 'squad_posts_workout_daily_limit' and not tgisinternal
  ) then missing := missing || ' trigger:squad_posts_workout_daily_limit'; end if;

  if not exists (
    select 1 from information_schema.columns
     where table_name = 'planned_workouts' and column_name = 'source_post_id'
  ) then missing := missing || ' planned_workouts.source_post_id'; end if;

  if not exists (
    select 1 from information_schema.columns
     where table_name = 'planned_workouts' and column_name = 'source_squad_id'
  ) then missing := missing || ' planned_workouts.source_squad_id'; end if;

  if not exists (
    select 1 from information_schema.columns
     where table_name = 'planned_workouts' and column_name = 'source_author_id'
  ) then missing := missing || ' planned_workouts.source_author_id'; end if;

  if not exists (
    select 1 from pg_proc p join pg_namespace n on n.oid = p.pronamespace
     where n.nspname = 'public' and p.proname = 'take_posted_workout'
  ) then missing := missing || ' fn:take_posted_workout'; end if;

  -- ⚠ It must be INVOKER. If a later hand "fixes" it to definer, the membership check silently stops
  -- being RLS's job and this function becomes the only thing between a stranger's post and a Home screen.
  if exists (
    select 1 from pg_proc p join pg_namespace n on n.oid = p.pronamespace
     where n.nspname = 'public' and p.proname = 'take_posted_workout' and p.prosecdef
  ) then missing := missing || ' fn:take_posted_workout/IS-DEFINER-SHOULD-BE-INVOKER'; end if;

  if missing <> '' then
    raise exception '0192 DID NOT FULLY APPLY. Missing or wrong:%', missing;
  end if;

  raise notice '0192 OK — type, trigger, three columns and the function are all present and correct.';
end $$;
...
select
  (select count(*) from public.squad_posts where type = 'workout')                      as workout_posts,
  (select count(*) from public.planned_workouts where source_post_id is not null)       as taken_slots,
  (select count(*) from public.planned_workouts)                                        as planned_slots_total,
  (select exists (select 1 from pg_constraint
                   where conname = 'squad_posts_type_check'
                     and pg_get_constraintdef(oid) like '%''workout''%'))                as type_allows_workout,
  (select exists (select 1 from pg_trigger
                   where tgname = 'squad_posts_workout_daily_limit' and not tgisinternal)) as trigger_present,
  (select exists (select 1 from pg_proc p join pg_namespace n on n.oid = p.pronamespace
                   where n.nspname = 'public' and p.proname = 'take_posted_workout'))    as function_present,
  (select not p.prosecdef from pg_proc p join pg_namespace n on n.oid = p.pronamespace
    where n.nspname = 'public' and p.proname = 'take_posted_workout' limit 1)            as function_is_invoker;
