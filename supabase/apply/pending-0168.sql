-- ═══════════════════════════════════════════════════════════════════════════════════════════════
-- Forge Legacy — PASTE-READY BUNDLE: migration 0168 (the competition that stopped moving)
--
...
-- WHAT IT DOES:
--
--   1. Restores `advance_challenges()` to 0087's FRIENDS-aware body. Under 0059's body — which is
--      SQUAD-ONLY, and which a re-paste of 0059 silently reinstates — a friends competition is never
--      promoted to ACTIVE and never completed. The days do not progress, and no winner is ever written.
--
--   2. ASSERTS both `advance_challenges` and `challenge_hub` still know FRIENDS exists. 0165 hardened
--      four of the six objects 0059 and 0087 share; these are the two it missed, and they are the two
--      that govern whether a competition moves at all.
--
--   3. ⭐ ENDS WITH A READ-ONLY REPORT of every competition's state, dates and result count. Nothing
--      in the app can show this. **Please copy the result table back** — the `due` column says
--      outright whether a competition is overdue to start, overdue to finish, or simply not there yet.
--
-- ⚠ SAFE TO RUN TWICE, AND SAFE TO RUN AT ANY TIME. It replaces one function body with the one the
--   repo already holds, and reads. It writes nothing to any table.
--
-- ⚠ IF §2 RAISES on `challenge_hub`, re-run `0163_competition_invite_and_standings.sql` — the hub's
--   newest body is 0163's, and copying it into this file would leave two copies to drift apart.
-- ═══════════════════════════════════════════════════════════════════════════════════════════════
