...
// ⚠ REVISION 5 (2026-09-24): FATSECRET IS THE THIRD BARCODE TRY (USDA → Open Food Facts → FatSecret),
// using Premier Free's `barcode` scope. Includes revision 4. Secrets set and IP allowlisted 2026-09-24.
// ═══════════════════════════════════════════════════════════════════════════════════════════

// ⚠ REVISION 7 (2026-09-24): fewer, better results, and a search that always answers. USDA household
// servings ("1 item", "1 banana") come first so Food Detail opens on the burger, not 100 g — looked up
// for the top 10 only; every typed word must match; plain and raw foods lead branded ones; the same
// food under two spellings or a plural shows once; FatSecret results are interleaved instead of sliced
// off; and every outbound call has a time limit, so one slow source can no longer blank the answer.
// ═══════════════════════════════════════════════════════════════════════════════════════════
// ⚠ REVISION 8 (2026-09-24): when USDA and FatSecret both answer empty, search falls back to foods
// already in the catalogue, never "Nothing found" for a food we hold. One log line per search
// (Dashboard → food-search → Logs) says what each source gave and how long it took.
// ═══════════════════════════════════════════════════════════════════════════════════════════
