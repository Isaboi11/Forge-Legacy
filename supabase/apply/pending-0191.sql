-- ═══════════════════════════════════════════════════════════════════════════════════════════════
-- PENDING — 0191: the weekly review can finally see the week it is describing
--
-- PASTE THIS WHOLE FILE into the Supabase SQL editor and run it once. Safe to run twice.
--
-- ══ WHAT IT DOES ══
--
-- Adds four fields to the `review` jsonb `ensure_weekly_review()` writes. No new columns, no new table,
-- no backfill:
--
--   top_lift.day      "Wednesday - Back Squat, 225 x 5" is a memory. Without the day it is a statistic.
--   sessions[]        {name, day, duration_sec} - the week as WHAT YOU DID, not HOW MANY TIMES.
--   first_time[]      exercises done for the FIRST TIME EVER, capped at 6.
--   longest_session   the headline's fallback on a cardio-only week.
--
-- ══ ⚠ THE PREDICTION FOR §3, WRITTEN BEFORE RUNNING ══
--
--   fn_is_definer            true    unchanged from 0140
--   returns_new_fields       true    the source now mentions all four
--   reviews_total            N       however many rows exist; this migration writes none
--   reviews_with_new_fields  0   ←   MUST BE ZERO. Snapshots are FROZEN: `ensure_weekly_review()` hands
--                                    back a stored row untouched, and nothing here backfills. A non-zero
--                                    count would mean something is rewriting history.
--
-- ⚠ SO NOTHING VISIBLY CHANGES UNTIL A NEW WEEK TURNS OVER. The first review generated after this lands
-- carries the four fields; every review already written keeps the seven it was born with, forever. The
-- client treats all four as optional for exactly that reason. This is correct - a review is a record of
-- what that week was, not a document that improves as the app does.
--
-- ⚠ THE COLUMN IS `workout_name`, NOT `name`. 0091 shipped `w.name` against `workouts`, PL/pgSQL bound
-- it late so it raised at RUN time rather than at create time, and 0094 exists solely to fix it.
-- ═══════════════════════════════════════════════════════════════════════════════════════════════

-- §1 · THE MIGRATION — verbatim from 0191_weekly_review_story_fields.sql
...
begin
  ...
  if v_src is null then
    raise exception '0191: ensure_weekly_review does not exist - 0140 was never applied?';
  end if;
  if v_src not like '%first_time%' then
    raise exception '0191: the replace did not take - the stored body has no first_time';
  end if;
  if v_src not like '%longest_session%' or v_src not like '%sessions%' then
    raise exception '0191: the stored body is missing sessions/longest_session';
  end if;
  -- ⚠ The trap 0094 exists for. If this ever reads `w.name` the function compiles and dies at RUN time.
  if v_src like '%w.name%' then
    raise exception '0191: the body references w.name - workouts has no such column (see 0094)';
  end if;
end;
$assert$;
...
-- ═════════════════════════════════════════════════════════════════════════════
--
-- ⚠ ONE ROW SET ON PURPOSE - the editor shows only the LAST statement's output.

select
  (select p.prosecdef from pg_proc p join pg_namespace n on n.oid = p.pronamespace
    where n.nspname = 'public' and p.proname = 'ensure_weekly_review')            as fn_is_definer,
  (select p.prosrc like '%first_time%' and p.prosrc like '%longest_session%'
     from pg_proc p join pg_namespace n on n.oid = p.pronamespace
    where n.nspname = 'public' and p.proname = 'ensure_weekly_review')            as returns_new_fields,
  (select count(*) from public.athlete_weekly_reviews)                            as reviews_total,
  -- MUST BE 0. Nothing here backfills, and a review is never rewritten.
  (select count(*) from public.athlete_weekly_reviews where review ? 'first_time') as reviews_with_new_fields;
