-- ════════════════════════════════════════════════════════════════════════════════════════════════
-- PENDING — 0182: a coach is somebody an athlete let in
--
-- PASTE THIS WHOLE FILE into the Supabase SQL editor and run it once.
-- Safe to run twice: every statement is guarded, and §3 is read-only.
--
-- This is the first migration of Forge Coach Phase B, governed by
-- `Docs/Forge-Coach-Architecture-v1.0.md`. Nothing before it in the ledger relates to it.
-- ════════════════════════════════════════════════════════════════════════════════════════════════
--
-- ══ WHAT THIS IS FOR ══
--
-- Forge Coach is a CRM for a HUMAN personal trainer with paying clients. **It is not Coach Holt.**
-- Holt is the in-app AI and already owns `/coach`, `coach_ai`, migrations 0138/0143/0144 and
-- `src/domain/coach/**`. FC-D2: every identifier here is `trainer_*`.
--
-- This migration builds ONLY the relationship — who holds a seat, and which athletes let one in.
-- It reads nobody's training data. The coach-side data RPCs come in a later migration, and every
-- one of them will call `trainer_client_guard()` as its first statement.
--
-- ══ THE ONE IDEA ══
--
-- FC-D4, and it is the whole security argument: **a coach read is a clearance the ATHLETE grants,
-- never a capability the coach holds.** `Squads-Hub-Wireframe-Spec-S1.md` §10.3 says the Performance
-- Firewall is "not overridden for premium users, coaches, or squad admins", and Squad Amendment 001
-- says "Consent, not privilege, is the gate." Modelled this way, §10.3 needs no amendment.
--
-- The trainer INVITES by exact handle, the athlete ACCEPTS, either may end it, and exactly one
-- relationship may be active per athlete at a time (FC-D6 — forced by the one-active-program rule).
--
-- ══ WHAT THIS FILE DOES ══
--
-- §1  creates `trainers` (seat register, RLS with ZERO policies) and `trainer_clients` (the consent
--     record, SELECT-only for either party), two partial unique indexes, and eight functions:
--     `is_trainer`, `trainer_guard`, `trainer_client_guard`, `trainer_invite_client`,
--     `athlete_respond_to_coach`, `athlete_revoke_coach`, `trainer_end_client`,
--     `trainer_withdraw_invite`.
-- §2  asserts every one of them exists, AND asserts the two RLS shapes that are the actual security
--     boundary: `trainers` has zero policies, `trainer_clients` has exactly one and it is a SELECT.
--     RAISES if any of it is wrong.
-- §3  reports what landed and how many rows exist. Read-only.
--
-- Additive and idempotent. Creates two new tables; touches no existing table, column or policy.
--
-- ⚠ THE ZERO-POLICY TABLE IS NOT A MISTAKE. `trainers` has RLS enabled and no policies, exactly as
-- `app_admins` does in 0129 and for the same reason: `profiles` is world-readable, and the list of
-- who holds a privileged seat must never become joinable to it. A later migration must not "fix"
-- this by adding an owner policy. §2 asserts the count is still zero.
--
-- ⚠ `trainer_clients` HAS NO WRITE POLICY, ALSO DELIBERATELY. The six RPCs are SECURITY DEFINER and
-- are the only writers. A client that could write this table could grant itself a coaching
-- relationship — and a coaching relationship is a read of somebody else's body. Same reasoning as
-- `athlete_usage` in 0145.
--
-- ⚠ APPLYING IS NOT THE SAME AS WORKING. No client code calls any of this yet, and there is no UI.
-- Expect §3 to report **0 trainers and 0 relationships** — that is correct, not a failed migration.
-- A non-zero count would mean something is writing tables that nothing should be writing yet.
-- Seating the first trainer is a hand-written INSERT; the recipe is at the foot of §1.
-- ════════════════════════════════════════════════════════════════════════════════════════════════
...
begin
  ...
  if to_regclass('public.trainers') is null then
    missing := missing || ' table:trainers'; end if;
  if to_regclass('public.trainer_clients') is null then
    missing := missing || ' table:trainer_clients'; end if;

  -- The two partial unique indexes. FC-D6 lives in the first one; a plpgsql check is a race.
  if to_regclass('public.trainer_clients_one_active_coach') is null then
    missing := missing || ' index:one_active_coach'; end if;
  if to_regclass('public.trainer_clients_one_live_pair') is null then
    missing := missing || ' index:one_live_pair'; end if;

  if not exists (select 1 from pg_constraint where conname = 'trainer_clients_not_self') then
    missing := missing || ' constraint:not_self'; end if;

  -- All seven functions, by exact signature.
  foreach v_fn in array array[
    'is_trainer()',
    'trainer_guard()',
    'trainer_client_guard(uuid)',
    'trainer_invite_client(text)',
    'athlete_respond_to_coach(uuid,boolean)',
    'athlete_revoke_coach()',
    'trainer_end_client(uuid)',
    'trainer_withdraw_invite(uuid)'
  ] loop
    if to_regprocedure('public.' || v_fn) is null then
      missing := missing || ' fn:' || v_fn;
    end if;
  end loop;

  -- ⚠ THE SECURITY BOUNDARY. These four are the assertions that matter: everything above could be
  -- present and correct while both tables sat wide open.
  if not (select c.relrowsecurity from pg_class c where c.oid = 'public.trainers'::regclass) then
    missing := missing || ' RLS-OFF:trainers'; end if;
  if not (select c.relrowsecurity from pg_class c where c.oid = 'public.trainer_clients'::regclass) then
    missing := missing || ' RLS-OFF:trainer_clients'; end if;

  select count(*) into v_pol from pg_policies pp
   where pp.schemaname = 'public' and pp.tablename = 'trainers';
  if v_pol <> 0 then
    missing := missing || ' trainers-HAS-POLICIES:' || v_pol; end if;

  select count(*) into v_pol from pg_policies pp
   where pp.schemaname = 'public' and pp.tablename = 'trainer_clients' and pp.cmd <> 'SELECT';
  if v_pol <> 0 then
    missing := missing || ' trainer_clients-HAS-WRITE-POLICY:' || v_pol; end if;

  if missing <> '' then
    raise exception '0182 DID NOT FULLY APPLY. Missing or wrong:%', missing;
  end if;

  raise notice '0182 OK — 2 tables, 2 partial indexes, 8 functions, and both RLS shapes are correct.';
end
$assert$;
...
-- The eight functions and how they are secured. Every one should say `definer`.
select p.proname                                 as function,
       pg_get_function_identity_arguments(p.oid) as args,
       case when p.prosecdef then 'definer' else 'INVOKER  <-- WRONG' end as security
from pg_proc p
join pg_namespace n on n.oid = p.pronamespace
where n.nspname = 'public'
  and p.proname in ('is_trainer', 'trainer_guard', 'trainer_client_guard',
                    'trainer_invite_client', 'athlete_respond_to_coach',
                    'athlete_revoke_coach', 'trainer_end_client',
                    'trainer_withdraw_invite')
order by p.proname;

-- ⚠ EXPECTED: trainers = 0 policies, trainer_clients = 1 policy. Both rls_enabled = true.
--   Any other number means the security boundary moved.
...
where n.nspname = 'public' and c.relname in ('trainers', 'trainer_clients')
order by c.relname;

-- ⚠ ALL THREE COUNTS SHOULD BE 0. Nothing in the app writes these tables yet, and no trainer has
--   been seated by hand. A non-zero number here would mean something is writing them that should not.
select (select count(*) from public.trainers)                                as trainers_seated,
       (select count(*) from public.trainer_clients)                         as relationships_total,
       (select count(*) from public.trainer_clients where status = 'active') as relationships_active;
