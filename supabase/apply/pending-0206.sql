...
--   • my_entitlement_gated: **true**.
--
-- ⚠ NOTHING IN §2 OR §3 CALLS `my_entitlement()`, AND THAT IS DELIBERATE. It raises
-- `28000 my_entitlement: no authenticated athlete` when `auth.uid()` is null, and the SQL editor runs as
-- `postgres`, which has none. An earlier draft called it: the migration COMMITTED correctly and then §2
-- aborted on that call, which looks exactly like a failed migration when nothing was wrong. The checks
-- read `pg_get_functiondef` instead, which works from any role.
...
begin
  ...
  -- ⚠ DO NOT CALL `my_entitlement()` HERE. It raises `28000 no authenticated athlete` when
  -- `auth.uid()` is null, and the SQL editor runs as `postgres`, which has no `auth.uid()`. An earlier
  -- draft called it and aborted this whole block — AFTER the migration had already committed, which
  -- reads as a failed migration when nothing was wrong. Read the function's SOURCE instead; that works
  -- from any role. (`has_nutrition_access()` is safe to call — it returns false rather than raising.)
  if not exists (
...
select (select count(*) from public.nutrition_preview) as allowlist_rows,
       ...
       -- Source, not a call: see the note in §2. Calling it as `postgres` raises 28000.
       ...
       -- Safe to call — returns false rather than raising. FALSE is expected in the SQL editor.
