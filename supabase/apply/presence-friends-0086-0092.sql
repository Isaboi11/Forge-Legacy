-- Forge Legacy - apply bundle: 0086 through 0092, IN ORDER.
--
-- NOT new migrations. All seven are numbered in supabase/migrations/; this file exists so the whole
-- chain can be pasted in one go.
--
-- ORDER IS NOT OPTIONAL. 0087 adds invited_ids, which 0088 reads; 0089 reads training_since from 0086;
-- 0090 replaces a function 0085 created; 0092 references workout_templates from 0091, and rebuilds the
-- notification functions 0088 last wrote. Out of order fails with 42703 or 42P01.
--
-- SAFE TO RE-RUN throughout: columns are `if not exists`, constraints and policies are dropped before
-- being recreated, and every function is create-or-replace (or explicitly dropped first where its
-- return type changed).
