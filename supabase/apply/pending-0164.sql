-- ═══════════════════════════════════════════════════════════════════════════════════════════════
-- Forge Legacy — PASTE-READY BUNDLE: migration 0164 (the competition invite reaches a phone, and
-- accepting one tells the person who sent it)
--
...
--
-- WHAT IT ADDS — the two halves 0163 deliberately left alone, approved as a P-5 amendment:
--
--   1. `challenge_updates` (off) is RETIRED and `challenge_invites` (ON) replaces it. The old toggle
--      read "Invitations and standing changes" and there are no standing-change notifications — so an
--      ambient-sounding label that described nothing was silencing an invitation aimed at you.
--
--   2. BRANCH 17, `challenge_joined`: somebody opted into a competition you created. There was no
--      such event anywhere, push or inbox — the creator found out by counting the roster.
--
-- ⚠ RUN AFTER 0163. It rebuilds `notification_events_for` from 0163's body (seventeen branches out),
-- and `push_pref_key` / `push_pref_default` from 0159's. Applying it to a database on an older copy
-- rolls those back for every other notification — 0088, 0092 and 0106 each shipped that failure.
--
-- ⚠ IT DOES NOT REACH BACKWARDS. The triggers fire on a row event and nothing re-scans, so this does
-- not deliver an invitation that was already sent. Make a new competition to see it.
--
-- SAFE TO RUN TWICE: every function is `create or replace`, the trigger is dropped before it is
-- created, and the preference carry-forward matches nothing on a second run.
--
-- VERIFY AFTER RUNNING — the migration self-checks, so a clean run already proves most of it:
--   select public.push_pref_key('challenge_invite');       -- challenge_invites
--   select public.push_pref_key('challenge_joined');       -- challenge_invites
--   select public.push_pref_default('challenge_invites');  -- true
--   select public.push_pref_default('squad_training');     -- true   ← 0153 survived the restatement
--   select public.push_pref_default('training_briefing');  -- false  ← 0159 survived it too
--
-- THEN THE REAL TEST, WITH A SECOND ACCOUNT:
--   1. Create a competition and name a friend.
--   2. On their device: the invite arrives as a push, and as a row in /inbox.
--   3. They tap Join.
--   4. On yours: "They're in — <name> joined <competition>".
--   5. select kind, title, body, status from public.push_outbox
--       where kind in ('challenge_invite', 'challenge_joined') order by created_at desc limit 5;
--
--   Nothing arrives on a device that has never registered a push token, or before the account's
--   `push_baseline_at` — that is the sender's floor, not a fault.
-- ═══════════════════════════════════════════════════════════════════════════════════════════════
