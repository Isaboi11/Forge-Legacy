-- ═══════════════════════════════════════════════════════════════════════════════════════════════
-- Forge Legacy — PASTE-READY BUNDLE: migration 0159 (the morning briefing)
--
...
--
-- WHAT IT ADDS: an opt-in morning push telling the athlete what their next session is. The FIRST
-- time-triggered notification in the app, and the first one not caused by another person — so it
-- deliberately does not join `notification_events_for`. It writes `push_outbox` directly, the way
-- 0137's operator alert does, and `forge-push-drain` sends it like everything else.
--
-- ⚠ RUN AFTER 0153. This restates `push_pref_key` and `push_pref_default` from 0153's bodies with
-- one key added. Applying it on a database still on an older copy would roll those two functions
-- BACK for every other notification — the exact failure 0088, 0092 and 0106 each shipped.
--
-- ⚠ IT CHANGES NOTHING UNTIL SOMEBODY TURNS IT ON. `training_briefing` defaults to false and
-- `briefing_schedule` is created empty, so applying this to the live database sends zero
-- notifications to anybody. It is safe to run at any hour of any day.
--
-- SAFE TO RUN TWICE: every table is `create table if not exists`, every function is
-- `create or replace`, all four policies are dropped before they are created, the copy seed is
-- `on conflict do nothing`, and the cron job is unscheduled before it is scheduled.
--
...
--
-- THEN THE REAL TEST, ON YOUR OWN ACCOUNT:
--   1. Account Settings → Notifications → Morning Briefing ON, pick today, pick the NEXT hour.
--   2. Wait for the hour to turn over (the job runs at :00, :15, :30, :45).
--   3. select kind, title, body, status from public.push_outbox
--       where kind = 'training_briefing' order by created_at desc limit 3;
--   4. It arrives on the phone and opens Home.
--
--   Nothing arrives if you have no active program AND no planned workout — that is the feature
--   (silence beats inventing a reason to ping), not a fault.
-- ═══════════════════════════════════════════════════════════════════════════════════════════════
