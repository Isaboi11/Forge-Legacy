-- ═══════════════════════════════════════════════════════════════════════════════════════════════
-- PENDING — 0214: App Store purchases reach the database, Early Bird seats, and who sees which offer
--
...
-- ══ WHAT THIS IS FOR ══
--
-- RevenueCat is set up (2026-09-24). This is the database half: the `revenuecat-webhook` Edge Function
-- calls `apply_store_event()` for every purchase, renewal, cancellation, expiration and refund, and the
-- app asks `my_paywall_offer()` which prices to show (regular, Early Bird, or the testers' AI add-on).
-- Full reasoning in the header of `supabase/migrations/0214_store_subscriptions.sql`.
--
-- ══ ⚠ ONE BEHAVIOUR CHANGE TODAY ══
--
-- The Premium AI switch on the Subscription screen (0203) becomes ADMINS-ONLY. With Premium AI on sale,
-- any account that could switch itself on would have it free. The PO is an admin and keeps the switch;
-- anyone else who taps it gets "Couldn't change Premium AI." Nothing else changes for anyone today:
-- no store event has arrived yet, so every athlete's entitlement is exactly what it was.
--
-- ══ WHAT THIS FILE DOES ══
--
-- §1  the migration, verbatim (2 tables, 1 column, 8 functions, grants)
-- §2  asserts every table, column and function exists, and the webhook function is service-role only
-- §3  reports what is there. Read-only.
--
-- Tested before handing over: every scenario (purchase, trial, cancel, refund, out-of-order and
-- duplicate delivery, seat claim + release, sandbox, Tester AI, transfer, re-run) against a local
-- Postgres with 0145's tables copied verbatim.
--
-- ══ PREDICTED §3 OUTPUT ══
--
--   store_subscriptions_rows   0     (no webhook deliveries yet)
--   store_events_rows          0
--   comped_testers             0     (set by `set-comped-testers-0214.sql`, a separate paste)
--   seats_remaining            100
--   premium_ai_switch          admins-only
-- ═══════════════════════════════════════════════════════════════════════════════════════════════


-- ═════════════════════════════════════════════════════════════════════════════
-- §1 — THE MIGRATION (verbatim from supabase/migrations/0214_store_subscriptions.sql)
-- ═════════════════════════════════════════════════════════════════════════════
...
declare
  f text;
begin
  if to_regclass('public.store_subscriptions') is null then raise exception '0214: store_subscriptions missing'; end if;
  if to_regclass('public.store_events') is null then raise exception '0214: store_events missing'; end if;
  if not exists (select 1 from information_schema.columns
                  where table_schema = 'public' and table_name = 'athlete_entitlement' and column_name = 'comped_tester') then
    raise exception '0214: athlete_entitlement.comped_tester missing';
  end if;
  foreach f in array array['store_athlete_for', 'store_product_grants', 'recompute_store_entitlement', 'claim_early_bird_seat',
                           'release_early_bird_seat', 'apply_store_event', 'my_paywall_offer', 'set_my_premium_ai'] loop
    if not exists (select 1 from pg_proc p join pg_namespace n on n.oid = p.pronamespace
                    where n.nspname = 'public' and p.proname = f) then
      raise exception '0214: function % missing', f;
    end if;
  end loop;
  if has_function_privilege('authenticated', 'public.apply_store_event(jsonb)', 'execute')
     or has_function_privilege('anon', 'public.apply_store_event(jsonb)', 'execute') then
    raise exception '0214: apply_store_event is callable by the app — it must be service-role only';
  end if;
  if not has_function_privilege('authenticated', 'public.my_paywall_offer()', 'execute') then
    raise exception '0214: my_paywall_offer is not callable by signed-in athletes';
  end if;
  if position('0214' in pg_get_functiondef('public.set_my_premium_ai(boolean)'::regprocedure)) = 0 then
    raise exception '0214: set_my_premium_ai is still the self-serve version';
  end if;
end $$;
...
select
  (select count(*) from public.store_subscriptions)                           as store_subscriptions_rows,
  (select count(*) from public.store_events)                                  as store_events_rows,
  (select count(*) from public.athlete_entitlement where comped_tester)       as comped_testers,
  public.founder_seats_remaining()                                            as seats_remaining,
  case when position('0214' in pg_get_functiondef('public.set_my_premium_ai(boolean)'::regprocedure)) > 0
       then 'admins-only' else 'STILL SELF-SERVE' end                         as premium_ai_switch;
