-- Forge Legacy — 0018: attribute a logged workout to the program it came from
--
-- Without this there is no way to answer "how far into this program am I" — the Program Detail screen's
-- progress bar, "Your Log" and stat tiles all reduce to counting THIS program's saved workouts. 0017
-- added the lifecycle; this closes the loop.
--
-- `on delete set null`, not cascade: deleting a program must never delete the sessions the athlete
-- actually trained. The workout survives, it just stops being attributed. That is the Never Charge For
-- History principle applied to deletes — history outlives the plan that produced it.

alter table public.workouts
  add column if not exists program_id uuid references public.programs(id) on delete set null;

create index if not exists workouts_program_saved
  on public.workouts (program_id, started_at)
  where program_id is not null;

-- save_workout gains p_program_id. The old 7-arg signature is dropped rather than left alongside, so
-- there is exactly one version of the Finish commit and no ambiguity about which one a client hit.
drop function if exists save_workout(text, modality, timestamptz, integer, text, jsonb, jsonb);
...
begin
  ...
  insert into workouts (athlete_id, chapter_id, program_id, workout_name, activity_type, started_at, saved_at, duration_sec, state, notes)
  values (v_uid, v_chapter, v_program, p_workout_name, p_activity_type, p_started_at, now(), p_duration_sec, 'saved', p_notes)
