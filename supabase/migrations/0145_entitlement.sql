...
--
-- ══ ⚠ PHASE F PREREQUISITE — BACKFILL `programs_created`, OR EVERY EXISTING ATHLETE GETS 3 FREE SLOTS ══
--
-- `athlete_usage.programs_created` starts at **0** for everybody, and the trigger only counts inserts made
-- from the moment this migration was applied. **Programs that already existed are invisible to it.**
--
-- That is harmless through Phases B–E, because the paid cap is 500 and nobody is near it. It is NOT
-- harmless at Phase F: an athlete already holding 3 programs would read `programs_created = 0` and be
-- handed three more, which is the free tier given away twice to exactly the people who have used the
-- product most. MA3-D9 says the count is LIFETIME; a counter that starts at zero on a populated account
-- is not a lifetime count.
--
-- Run this BEFORE flipping `default_tier`. Idempotent — `greatest` means re-running never lowers a count
-- that the trigger has since carried past the row total.
--
--   insert into public.athlete_usage (athlete_id, programs_created)
--   select p.athlete_id, count(*)
--     from public.programs p
--    group by p.athlete_id
--   on conflict (athlete_id) do update
--      set programs_created = greatest(public.athlete_usage.programs_created, excluded.programs_created),
--          updated_at = now();
--
-- ⚠ IT UNDER-COUNTS ON PURPOSE, AND THAT IS THE RIGHT DIRECTION. A program created and then DELETED
--   before this migration existed left no row and cannot be recovered — no soft delete, no audit trail.
--   So an athlete who churned through six programs and kept none reads 0 and gets their three. Granting a
--   slot nobody can prove was spent is the kinder error; refusing one on a number we invented is not.
--
-- ⚠ `has_used_free_import` CANNOT BE BACKFILLED AT ALL. Nothing in the schema has ever recorded that an
--   import happened — the flag had zero occurrences in code or SQL before this file. Every athlete
--   therefore starts with their free import unspent, including anyone who has already used one. Accepted:
--   it errs toward the athlete, and there is no data to do better with.
--
-- ══ PHASE F — THE SWITCH-ON, IN FULL ══
--
--   -- 0. Backfill first (above), then:
